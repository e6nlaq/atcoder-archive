...
int main() {
    ...
    rep(i, N) {
        ...
        vll hash(S.size());
        hash[0] = S[0] - 'a' + 1;
        arep(i, 1, S.size()) hash[i] = (hash[i - 1] * b + (S[i] - 'a' + 1)) % m;

        debug(hash);

        rep(i, S.size()) {
            if (hashs.contains(hash[i])) {
                chmin(ans, S.size() - i - 1 + hashs[hash[i]]);
            }
        }
        ...
        rep(i, S.size()) {
            if (hashs.contains(hash[i]))
                chmin(hashs[hash[i]], S.size() - i - 1);
            else
                hashs[hash[i]] = S.size() - i - 1;
        }
    }
