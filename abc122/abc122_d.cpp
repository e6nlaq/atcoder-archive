...
int main() {
    ...
    // AGCT
    auto dp = make_vec2<vec2<mint>>(N + 1, 4, make_vec2<mint>(4, 4, 0));
    dp[0][3][3][3] = 1;
    ...
    irep(i, N) {
        rep(c, 4) {
            rep(f, 4) {
                rep(g, 4) {
                    rep(h, 4) {
                        if (ng.contains(vll{f, g, h, c})) continue;

                        dp[i][g][h][c] += dp[i - 1][f][g][h];
                    }
                }
            }
        }
