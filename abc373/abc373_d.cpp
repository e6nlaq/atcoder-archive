...
void dfs(ll x) {
    ...
    bool shutan = !kakutei[x];
    rep(i, G[x].size()) {
        shutan &= check[G[x][i].to] && !kakutei[G[x][i].to];
    }
    ...
    //             dfs(G[x][i].to);
    //         }
    //         assert(kakutei[G[x][i].to]);

    //         ans[x] = ans[G[x][i].to] - G[x][i].cost;
    //         kakutei[x] = true;
    //     } else {
    //         if (!kakutei[G[x][i].to]) {
    //             ans[G[x][i].to] = ans[x] + G[x][i].cost;
    //             kakutei[G[x][i].to] = true;
...
void ume(ll x) {
    ...
    rep(i, ryo[x].size()) {
        if (!kakutei[ryo[x][i].to]) {
            ll to = ryo[x][i].to;
            ans[to] = ans[x] + ryo[x][i].cost;
            kakutei[to] = true;
            ume(to);
        }
    }
...
int main() {
    ...
    rep(i, M) {
        ...
        G[u].emplace_back(make_cost(v, w));
        ryo[u].emplace_back(make_cost(v, w));
        ryo[v].emplace_back(make_cost(u, -w));
    }
