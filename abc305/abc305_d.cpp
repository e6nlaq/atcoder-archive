...
int main()
{
	...
	while (Q--)
	{
		...
		auto li = lower_bound(all(A), l);
		auto ri = lower_bound(all(A), r);

		ll la = li - A.begin(), ra = ri - A.begin();

		cerr << endl;
		debug(l, r);
		debug(la, ra);
		debug(A[la], A[ra], tim[la], tim[ra]);
		sum = 0;
		sum += tim[ra] - tim[la];
		debug(sum);
