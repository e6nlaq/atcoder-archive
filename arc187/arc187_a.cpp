...
int main() {
    ...
    ll cnt = 0;
    bool ansok = true;
    while (ans.size() <= 500000 && cnt <= 100000 && !is_sorted(all(A))) {
        bool kokomade = true;
        rep(i, N - 1) {
            ll moto = (i != 0 ? A[i - 1] : -INFLL);

            kokomade &= A[i] <= A[i + 1];

            if (kokomade) continue;
            ...
            if (A[i] > A[i + 1] && A[i] - A[i + 1] < K) {
                if (i != N - 2) {
                    ...
                    while (abs(A[i] - A[i + 1]) < K || A[i - 1] > A[i]) {
                        swap(A[i], A[i - 1]);
                        A[i - 1] += K;
                        ans.emplace_back(i);
                    }
            ...
            while ((!(A[i] <= A[i + 1] && moto <= A[i] && moto <= A[i + 1])) && ans.size() <= 500000) {
                swap(A[i], A[i + 1]);
                A[i] += K;
    ...
    if (is_sorted(all(A)) && ans.size() <= 500000) {
        co("Yes");
        co(ans.size());
        print(ans);
    } else {
