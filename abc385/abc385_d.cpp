...
int main() {
    ...
    map<ll, set<pll>> w;  // 各hに対する家(w)
    map<ll, set<pll>> h;  // 各wに対する家(h)
    vll X(N), Y(N);
    rep(i, N) {
        cin >> X[i] >> Y[i];
        h[X[i]].insert({Y[i], i});
    ...
    rep(_cc, M) {
        ...
        if (D == 'U' || D == 'D') {
            ...
            auto sit = h[Sx].lower_bound(make_pair(l, -1LL));
            vpll er;
            for (auto it = sit; it != h[Sx].end() && it->first <= r; it++) {
                er.emplace_back(*it);
            }
            ans += er.size();
            rep(i, er.size()) {
                auto [y, at] = er[i];
                assert(w[y].contains(make_pair(Sx, at)));
                w[y].erase(make_pair(Sx, at));
                h[Sx].erase(er[i]);
            }
            ...
            auto sit = w[Sy].lower_bound(make_pair(l, -1LL));
            vpll er;
            for (auto it = sit; it != w[Sy].end() && it->first <= r; it++) {
                er.emplace_back(*it);
            }
            ans += er.size();
            rep(i, er.size()) {
                auto [x, at] = er[i];
                assert(h[x].contains(make_pair(Sy, at)));
                h[x].erase(make_pair(Sy, at));
                w[Sy].erase(er[i]);
            }
