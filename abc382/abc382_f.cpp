...
int main() {
    ...
    vector<tuple<ll, ll, ll, ll>> dat(N);
    rep(i, N) {
        ll R, C, L;
        cin >> R >> C >> L;

        dat[i] = make_tuple(R, C, L, i);
    }
    sort(rall(dat));
    ...
    vll seginit(W + 1, H + 1);
    lazy_segtree<S, op, e, F, mapping, composition, id> seg(seginit);

    vll ans(N);
    rep(i, N) {
        auto [R, C, L, x] = dat[i];
        ll pos = seg.prod(C, C + L);
        ans[x] = pos - 1;
        seg.apply(C, C + L, pos - 1);
    }
