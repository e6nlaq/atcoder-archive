...
int main() {
    ...
    vll dat;
    if (A[0] != 1) dat.emplace_back(A[0] - 1);
    arep(i, 1, M) {
        dat.emplace_back(A[i] - A[i - 1] - 1);
        if (dat.back() == 0) dat.pop_back();
    }
    if (A.back() != N) {
        dat.emplace_back(N - A.back());
    }

    debug(dat);

    ll mn = INFLL;
    rep(i, dat.size()) chmin(mn, dat[i]);

    ll ans = 0;
    rep(i, dat.size()) ans += dat[i] / mn + (dat[i] % mn != 0);
