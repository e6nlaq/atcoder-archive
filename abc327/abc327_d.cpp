...
vector<ll> check;
vector<vector<ll>> df(0, vector<ll>(0));

ll a(ll x) { return (x == 0 ? 1 : 0); }

void dfs(ll x, ll mot)
{
	// debug(x, mot);
	// assert(x >= 0 && x < N);
	check.at(x) = a(mot);
	rep(i, df.at(x).size())
	{
		if (check.at(df.at(x).at(i)) == a(mot))
		{
			co("No");
			exit;
		}
...
int main()
{
	...
	rep(i, M)
	{
		...
		// G.merge(A[i], B[i]);
		df[A[i]].emplace_back(B[i]);
		df[B[i]].emplace_back(A[i]);
