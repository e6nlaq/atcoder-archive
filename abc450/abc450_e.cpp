...
int main() {
    ...
    len[0] = X.size();
    len[1] = Y.size();
    while (len.back() < 1e18) len.emplace_back(len[len.size() - 2] + len[len.size() - 1]);
    debug(len, len.size());

    N = len.size();
    auto dat = make_vec2<ll>(26, N, 0);
    for (auto c : X) {
        dat[c - 'a'][0]++;
    }
    for (auto c : Y) dat[c - 'a'][1]++;

    rep(c, 26) {
        arep(i, 2, N) {
            dat[c][i] = dat[c][i - 1] + dat[c][i - 2];
        }
    }
    debug(dat);

    auto x = make_vec2<ll>(26, X.size(), 0);
    auto y = make_vec2<ll>(26, Y.size(), 0);
    rep(c, 26) {
        rep(i, X.size()) {
            x[c][i] = X[i] == c + 'a';
            if (i != 0) x[c][i] += x[c][i - 1];
        }
        rep(i, Y.size()) {
            y[c][i] = Y[i] == c + 'a';
            if (i != 0) y[c][i] += y[c][i - 1];
        }
    }

    // S_xのn文字目までにcが何個含まれるか
    // x=0-inde
    // n=1-inde
    auto f = [&](this auto f, int c, ll a, ll n) -> ll {
        if (a == 0) {
            return x[c][n - 1];
        }
        if (a == 1) {
        ...
        if (n <= len[a - 1]) {
            ...
            return f(c, a - 2, n - len[a - 1]) + dat[c][a - 1];
        }
    };
