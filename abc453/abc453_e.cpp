...
int main() {
    ...
    mint sol = 0;
    for (ll i = 0; i < N / 2; ++i) {
        for (auto x : dat[i]) fw.add(x, 1);

        if (N % 2 == 0 && i == N / 2 - 1) {
            if (inc[i] == N) sol += ncr.COM(N, i + 1);
        } else {
            ll ze = fw.sum(i, N - i - 2);
            if (ze > i + 1) continue;

            ll l = inc[i];
            ll r = inc[N - i - 2] - fw.sum(N - i - 2, N);

            if (l + r == N) ans += ncr.COM(l - ze, i + 1 - ze);
            debug(i, l, r, inc[i], N - i - 2, inc[N - i - 2], fw.sum(N - i - 2, N), ze);
        }
        debug(i, ans, sol);
    }
