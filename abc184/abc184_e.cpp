...
int main() {
    ...
    auto dist = make_vec2<ll>(H, W, INFLL);
    ll sx, sy, gx, gy;
    auto pos = make_vec2<pll>(26, 0);
    rep(i, H) {
        rep(j, W) {
            if (a[i][j] == 'S') {
                sx = i;
    ...
    while (!q.empty()) {
        ...
        rep(i, 4) {
            ...
            if (a[nx][ny] == '#') continue;
            if (dist[nx][ny] != INFLL) continue;

            dist[nx][ny] = dist[x][y] + 1;
            q.emplace(nx, ny);
        }

        if (islower(a[x][y]) && !checka[a[x][y] - 'a']) {
            checka[a[x][y] - 'a'] = true;
            for (auto [nx, ny] : pos[a[x][y] - 'a']) {
                if (dist[nx][ny] != INFLL) continue;
                dist[nx][ny] = dist[x][y] + 1;
                q.emplace(nx, ny);
            }
    ...
    cout << (dist[gx][gy] == INFLL ? -1 : dist[gx][gy]) << endl;
