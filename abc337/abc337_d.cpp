...
void solve(vector<vector<char>> s, ll K)
{
	ll H = s.size(), W = s[0].size();
	vector<vector<ll>> batu(H, vector<ll>(W, 0)), maru(H, vector<ll>(W, 0));
	rep(i, H)
	{
		rep(j, W)
		{
	...
	vector<vector<ll>> B(H), O(H);
	rep(i, H)
	{
		B[i] = cumulative(batu[i]);
		O[i] = cumulative(maru[i]);
	}
	debug(B, O);
	...
	for (int i = 0; i < H; ++i)
	{
		for (int j = 0; j < W; ++j)
		{
			...
			debug(O[i][j + K] - O[i][j], j + K);
			if (B[i][j + K] - B[i][j] == 0)
			{
				chmin(ans, max(0LL, K - (O[i][j + K] - O[i][j])));
			}
		}
	}
}
