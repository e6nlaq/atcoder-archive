...
int main()
{
	...
	auto dp = make_vec2<modint998244353>(N + 1, 2, 0);
	dp[0][0] = 1;

	irep(i, N)
	{
		dp[i][0] = (A[i - 1] == A[i] ? 0 : dp[i - 1][0]) + (B[i - 1] == A[i] ? 0 : dp[i - 1][1]);
		dp[i][1] = (A[i - 1] == B[i] ? 0 : dp[i - 1][0]) + (B[i - 1] == B[i] ? 0 : dp[i - 1][1]);
	}

	co((dp[N][0] + dp[N][1]).val());
