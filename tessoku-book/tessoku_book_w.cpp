...
int main()
{
	...
	vector<vector<ll>> dp(M + 1, vector<ll>((1 << N) + 1, INFLL));
	dp[0][0] = 0;
	...
	for (int i = 1; i <= M; ++i)
	{
		for (int j = 0; j < (1 << N); ++j)
		{
			...
			dp[i][ans] = min(dp[i][ans], dp[i - 1][j] + 1);
		}
	}

	debug(dp);

	co((dp[M][(1 << N) - 1] == INFLL ? -1 : dp[M][(1 << N) - 1]));
