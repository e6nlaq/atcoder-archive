...
inline vll make_hash(const vll &a) {
    HPOW.resize(a.size() + 1);
    HPOW[0] = 1;
    irep(i, a.size()) HPOW[i] = hmod_mul(HPOW[i - 1], HB);
...
inline vll make_hash(const string &s) {
    return make_hash(str_to_vec(s));
}
...
int main() {
    ...
    rep(_query, Q) {
        ...
        YesNo(get_hash(h, a - 1, b - 1) == get_hash(h, c - 1, d - 1));
    }
