...
int main() {
    ...
    vector<v> tate(M);  // name=key
    rep(i, M) {
        ll X, Y;
        char C;
        cin >> X >> Y >> C;

        tate[i] = make_tuple(X, Y, C);
    }
    ...
    rep(i, M) {
        auto [x, y, c] = tate[i];
        if (c == 'W' && !nutta.contains(x)) {
            nutta.emplace(x);
            dat.push_back({x, y - 1});
        }
    }

    arep(i, 1, dat.size()) chmin(dat[i].second, dat[i - 1].second);
    debug(dat);

    bool ans = true;
    rep(i, M) {
        auto [x, y, c] = tate[i];
        auto it = upper_bound(all(dat), make_pair(x, INFLL));
        it--;
        ll n = it->second;
        if (c == 'B' && n < y) {
            ans = false;
