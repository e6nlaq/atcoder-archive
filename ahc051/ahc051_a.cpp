...
constexpr char cl = '\0';
constexpr ll nl = 0LL;
constexpr ll INFINT = 20474473647;
...
CPP_DUMP_DEFINE_EXPORT_OBJECT(Point, x, y);

// 数値の符号を返す関数
// x > 0 なら 1, x < 0 なら -1, x == 0 なら 0 を返す
int sign(double x) {
    if (x > 0) return 1;
    if (x < 0) return -1;
    return 0;
...
bool segments_intersect(const Point &p1, const Point &p2, const Point &q1, const Point &q2) {
    ...
    // 各線分から見た相手の線分の端点の向きを調べる
    int o1 = orientation(p1, p2, q1);
    int o2 = orientation(p1, p2, q2);
    int o3 = orientation(q1, q2, p1);
    int o4 = orientation(q1, q2, p2);

    // 一般的な交差（両方の線分が互いをまたぐ場合）
    return (o1 * o2 < 0) && (o3 * o4 < 0);
}

vpll G_final;
pll emp = make_pair(-1, -1);
...
// DFS to check if a vertex has a descendant of type A
// memo: -1 = unvisited, 0 = false, 1 = true
bool has_A_descendant(ll u, const vpll& G, ll N, pll emp, vector<int>& memo) {
    if (u == -1) return false;
    // Base case: If u is a type A vertex
    if (u < N) {
        return true;
    }
    // Base case: If u is a type B vertex but has no children (leaf in the context of G)
    if (u >= N) {
        if (u - N >= G.size() || G[u - N] == emp) {
            return false;
        }
    }
    ...
    // It's a B-vertex (u-N) with children
    const auto& children = G[u - N];
    ll c1 = children.first;
    ll c2 = children.second;

    // Recursive step
    bool result = has_A_descendant(c1, G, N, emp, memo) || has_A_descendant(c2, G, N, emp, memo);
...
int main() {
    ...
    vector<Point> d(N), s(M);  // 処理設備, 分別
    rep(i, N) cin >> d[i].x >> d[i].y;
    rep(i, M) cin >> s[i].x >> s[i].y;

    vector<Point> ds = d;
    ds.insert(ds.end(), s.begin(), s.end());

    auto p = make_vec2<ld>(K, N);
    cin >> p;

    vvll G(M); // B-node i -> children
    G_final.resize(M, emp);
    ...
    {
        queue<ll> q;
        if(start != -1) q.emplace(start);

        vector<pair<Point, Point>> edges;
        vb check(ds.size(), false);
        if(start != -1) {
            check[start] = true;
            edges.emplace_back(make_pair(Point(0, 5000), ds[start]));
        }
        ...
        while(true) {
            while (!q.empty()) {
                ...
                if (now < N) continue;
                if (G[now - N].size() >= 2) continue;

                vector<pair<ll, ll>> a_cands, b_cands;
                rep(i, ds.size()) {
                    if (i == now || check[i]) continue;
                    bool is_intersect = false;
                    for (const auto &edge : edges) {
                        if (segments_intersect(ds[now], ds[i], edge.first, edge.second)) {
                            is_intersect = true;
                            break;
                        }
                    }
                    if (!is_intersect) {
                        ll dist_val = dist(ds[now], ds[i]);
                        if (i < N) a_cands.emplace_back(dist_val, i);
                        else b_cands.emplace_back(dist_val, i);
                    }
                }

                vector<pair<ll, ll>> cands;
                cands.insert(cands.end(), a_cands.begin(), a_cands.end());
                cands.insert(cands.end(), b_cands.begin(), b_cands.end());
                sort(cands.begin(), cands.end());

                int capacity = 2 - G[now - N].size();
                vector<ll> children_to_add;
                for(const auto& cand : cands) {
                    if (children_to_add.size() >= capacity) break;
                    children_to_add.push_back(cand.second);
                }

                for(ll child : children_to_add) {
                    G[now - N].push_back(child);
                    edges.emplace_back(ds[now], ds[child]);
                    if (!check[child]) {
                        check[child] = true;
                        q.push(child);
                    }
            ...
            rep(i, ds.size()) if(!check[i]) all_connected = false;
            if(all_connected) break;

            pair<ll, pair<ll, ll>> best_A_bridge = {INFLL, {-1, -1}};
            pair<ll, pair<ll, ll>> best_B_bridge = {INFLL, {-1, -1}};

            for (ll u = N; u < ds.size(); ++u) {
                if (!check[u] || G[u - N].size() >= 2) continue;
                for (ll v = 0; v < ds.size(); ++v) {
                    if (check[v]) continue;
                    
                    bool is_intersect = false;
                    for (const auto &edge : edges) {
                        if (segments_intersect(ds[u], ds[v], edge.first, edge.second)) {
                            is_intersect = true;
                            break;
                        }
                    }
                    ...
                    if (!is_intersect) {
                        ll d = dist(ds[u], ds[v]);
                        if (v < N) { // is A-node
                            if (d < best_A_bridge.first) best_A_bridge = {d, {u, v}};
                        } else { // is B-node
                            if (d < best_B_bridge.first) best_B_bridge = {d, {u, v}};
                        }
                    }
                }
            }

            pair<ll, pair<ll, ll>> bridge_to_add = {-1, {-1, -1}};
            if (best_A_bridge.second.first != -1) {
                bridge_to_add = best_A_bridge;
            } else if (best_B_bridge.second.first != -1) {
                bridge_to_add = best_B_bridge;
            }

            if (bridge_to_add.second.first != -1) {
                ll u = bridge_to_add.second.first;
                ll v = bridge_to_add.second.second;
                G[u - N].push_back(v);
                edges.emplace_back(ds[u], ds[v]);
                check[v] = true;
                q.push(v);
            } else {
                break; 
        ...
        // Post-process to give sorters with 1 child a second child (if possible)
        rep(i, M) {
            if (G[i].size() == 1) {
                ll u = i + N;
                ll c1 = G[i][0];
                
                pair<ll, ll> best_c2 = {-1, INFLL}; // {idx, dist}

                // Try to connect to an existing A-node
                for(ll j=0; j<N; ++j) {
                    if (j == c1) continue;
                    
                    bool is_intersect = false;
                    for (const auto &edge : edges) {
                        if (segments_intersect(ds[u], ds[j], edge.first, edge.second)) {
                            is_intersect = true;
                            break;
                        }
                    }
                ...
                if (best_c2.first != -1) {
                    G[i].push_back(best_c2.first);
                    edges.emplace_back(ds[u], ds[best_c2.first]);
                }
            }
        }
    }
    ...
    rep(i, M) {
        if (G[i].size() == 2) {
            G_final[i] = {G[i][0], G[i][1]};
        } else if (G[i].size() == 1) {
            G_final[i] = {G[i][0], G[i][0]};
        }
    }

    vector<int> memo(ds.size(), -1);
    rep(i, M) {
        if (G_final[i] == emp) continue;
        ll c1 = G_final[i].first;
        ll c2 = G_final[i].second;
        bool c1_has_A = has_A_descendant(c1, G_final, N, emp, memo);
        bool c2_has_A = has_A_descendant(c2, G_final, N, emp, memo);
        if (!c1_has_A && !c2_has_A) G_final[i] = emp;
        else if (c1_has_A && !c2_has_A) G_final[i].second = c1;
        else if (!c1_has_A && c2_has_A) G_final[i].first = c2;
    }

    vll k(M, 0);
    rep(i, M) k[i] = randint(0, K - 1);

    Graph adj(ds.size());
    rep(i, M) {
        if (G_final[i] != emp) {
            adj[i + N].push_back(G_final[i].first);
            if (G_final[i].first != G_final[i].second) {
                adj[i + N].push_back(G_final[i].second);
            }
        }
    }
    auto sorted_nodes = topological_sort(adj).sort;

    auto prob = make_vec2<ld>(ds.size(), N, 0.0);
    if (start != -1) {
        rep(j, N) prob[start][j] = 1.0;
    }

    for (ll u : sorted_nodes) {
        if (u >= N && G_final[u - N] != emp) {
            ll c1 = G_final[u - N].first;
            ll c2 = G_final[u - N].second;
            rep(j, N) {
                if (prob[u][j] > 0) {
                    if (c1 != -1) prob[c1][j] += prob[u][j] * p[k[u - N]][j];
                    if (c2 != -1) prob[c2][j] += prob[u][j] * (1.0 - p[k[u - N]][j]);
                }
            }
        }
    }
    ...
    vll D(N, -1);
    vb assigned_A(N, false);
    rep(j, N) {
        ll best_A = -1;
        ld max_prob = -1.0;
        rep(k_idx, N) {
            if (!assigned_A[k_idx] && prob[k_idx][j] > max_prob) {
                max_prob = prob[k_idx][j];
                best_A = k_idx;
            }
        }
        if (best_A != -1) {
            D[best_A] = j;
            assigned_A[best_A] = true;
        }
    }
    vll unassigned_trash;
    vb trash_is_assigned(N, false);
    rep(k_idx, N) if(D[k_idx] != -1) trash_is_assigned[D[k_idx]] = true;
    rep(j, N) if(!trash_is_assigned[j]) unassigned_trash.push_back(j);
    ll current_trash_idx = 0;
    rep(k_idx, N) {
        if (D[k_idx] == -1) {
            if(current_trash_idx < unassigned_trash.size()){
                 D[k_idx] = unassigned_trash[current_trash_idx++];
            }
        }
    }

    auto amount = make_vec2<ld>(ds.size(), N, 0.0);
    rep(i, N) {
        if (D[i] != -1) {
            amount[i][D[i]] = 1.0;
        }
    ...
    for(ll u : sorted_nodes) {
        if (u >= N) {
            ...
            if (c1 != -1) {
                rep(j, N) amount[u][j] += amount[c1][j];
            }
            if (c2 != -1 && c1 != c2) {
                rep(j, N) amount[u][j] += amount[c2][j];
            }
            ...
            rep(ki, K) {
                ld current_score = 0;
                rep(j, N) {
                    ld prob_c1 = 0, prob_c2 = 0;
                    if(c1 != -1) prob_c1 = amount[c1][j];
                    if(c2 != -1) prob_c2 = amount[c2][j];
                    
                    current_score += prob_c1 * p[ki][j] + prob_c2 * (1.0 - p[ki][j]);
                }
    ...
    rep(i, M) {
        if (G_final[i] == emp) {
            ...
            co(k[i], G_final[i].first, G_final[i].second);
        }
    }
