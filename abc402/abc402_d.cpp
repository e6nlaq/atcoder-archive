...
pair<pll, pll> solve(pll x) {
    ll d = abs(x.first - x.second);
    pll a;
    {
        ll k = d / 2;
        a = {x.first + k, x.second - k};
        form(a);
    }
    pll b;
    {
        ll k = (N - d) / 2;
        b = {x.first - k, x.second + k};
        debug(d, k, b);
        form(b);
    }
...
int main() {
    ...
    rep(i, M) {
        ...
        assert(!(dat.contains(a) && dat.contains(b)));
        if (!dat.contains(a) && !dat.contains(b)) {
            dat[a]++;
        } else if (dat.contains(a)) {
            dat[a]++;
        } else {
            dat[b]++;
        }
