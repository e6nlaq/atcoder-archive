...
int main() {
    ...
    lazy_segtree<S, op, e, F, mapping, composition, id> seg(l.size());
    seg.set(0, {0, 1});
    rep(i, Q) {
        ll q = qu[i][0];
        if (q == 1) {
            ll x = qu[i][1];
            seg.set(pos[i + 1], {i + 1, 1});
            ...
            ll py = pos[y];
            if (px > py) swap(px, py);
            co(seg.prod(px + 1, py).value);
            seg.apply(px + 1, py, 0);
        }
#ifdef LOCAL
        vll s(l.size());
        rep(j, l.size()) s[j] = seg.get(j).value;
        debug(i, s);
#endif
    }
