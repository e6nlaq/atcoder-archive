...
vpll poss = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
ll posr = 0;

string movstr(char top) {
    return string{top} + "."_s * (2 * useV - 1);
}

ll hvc() {
    ll cnt = 0;
    rep(i, hand) cnt += have[i];
...
void checkp() {
    ...
    // }
    rep(i, hand) {
        ll X = x + poss[(i + posr) % 4].first, Y = y + poss[(i + posr) % 4].second;
        debug(X, Y, have[i], i);
        if (!have[i] && tako.contains({X, Y})) {
            have[i] = true;
            pick(i + 1);
            tako.erase({X, Y});
        }

        if (have[i] && tar.contains({X, Y})) {
            have[i] = false;
            pick(i + 1);
            tar.erase({X, Y});
        }
...
void mov(ll nx, ll ny) {
    // debug(nx, ny);
    string tmp = "";
    while (x != nx) {
        ans.emplace_back(movstr(nx < x ? 'U' : 'D'));
    ...
    while (y != ny) {
        ans.emplace_back(movstr(ny < y ? 'L' : 'R'));
...
void rot() {
    ans.emplace_back("." + "R"_s * (useV - 1) + "."_s * (useV));
    // rep(i, useV - 1) {
    //     ans.back()[i + 1] = 'R';
    // }
    posr++;
...
void rotl(bool change = true) {
    ans.emplace_back("." + "L"_s * (useV - 1) + "."_s * (useV));
    // rep(i, useV - 1) {
    //     ans.back()[i + 1] = 'R';
    // }
...
int main() {
    ...
    rep(i, N) {
        rep(j, N) {
            ...
            if (s[i][j] == '1') tako.insert({i, j});

            if (t[i][j] == '1') tar.insert({i, j});
        }
    }

    // while (tako.contains({x, y})) {
    x = (*tako.begin()).first;
    y = (*tako.begin()).second;
    // }
    ...
    {
        const string tmp_fit = "LLRR"_s * 5;
        // co(".LLRR.....");
        cout << "." << tmp_fit.substr(0, hand) << "."_s * useV << endl;

        const string tmp_sec = ".R.R"_s * 5;
        cout << "." << tmp_sec.substr(0, hand) << "."_s * useV << endl;

        // co("..R.R.....");
    }

    // debug(tako.size(), tar.size());

    ll turn = 0;
    while ((!tako.empty() || !tar.empty()) && ans.size() < 99000 && turn < 500000) {
        // if (!(have[0] && have[1] && have[2] && have[3])) {
        do {
            for (ll turn = 0; turn < hand; ++turn) {
                if (!have[turn]) {
                    pll nex = {INFINT, INFINT};
                    ll tp = (turn + posr) % 4;
                    for (auto tmp : tako) {
                        ll nxp = tmp.first - poss[tp].first;
                        ll nyp = tmp.second - poss[tp].second;
                        if (dist({x, y}, nex) > dist({x, y}, tmp)) {
                            nex = tmp;
                        }
                    }

                    debug(tako.size(), have, nex, x, y, posr);
                    if (nex != make_pair(INFINT, INFINT)) {
                        ll nxp = nex.first - poss[tp].first;
                        ll nyp = nex.second - poss[tp].second;
                        ll rotcnt = 0;
                        while (!(nxp >= 0 && nxp < N && nyp >= 0 && nyp < N)) {
                            rot();
                            rotcnt++;
                            tp = (turn + posr) % 4;
                            nxp = nex.first - poss[tp].first;
                            nyp = nex.second - poss[tp].second;
                        }
        ...
        } while (tako.size() >= (ull)hand && hvc() != hand);
        // if (have[0] || have[1] || have[2] || have[3]) {
        bool orosita = !have[0] && !have[1] && !have[2] && !have[3];
        do {
            for (ll turn = 0; turn < hand; ++turn) {
                if (have[turn]) {
                    ll tp = (turn + posr) % 4;
                    ...
                    for (auto tmp : tar) {
                        ll nxp = tmp.first - poss[tp].first;
                        ll nyp = tmp.second - poss[tp].second;
                        if (dist({x, y}, t) > dist({x, y}, tmp)) {
                            t = tmp;
                        }
                    }

                    if (t != make_pair(INFINT, INFINT)) {
                        ll nxp = t.first - poss[tp].first;
                        ll nyp = t.second - poss[tp].second;
                        ll rotcnt = 0;
                        while (!(nxp >= 0 && nxp < N && nyp >= 0 && nyp < N)) {
                            rot();
                            rotcnt++;
                            tp = (turn + posr) % 4;
                            nxp = t.first - poss[tp].first;
                            nyp = t.second - poss[tp].second;
                        }
                        if (rotcnt == 3) {
                            rep(_, 3) ans.pop_back();
                            rotl(false);
                        }
                        mov(t.first - poss[tp].first, t.second - poss[tp].second);
                        // orosita = true;
                    }
                }
            }
        } while (tar.size() >= (ull)hand && hvc() != 0);

        // if (!orosita) {
        // rot();
        // }

        // mov(randint(0, N - 1), randint(0, N - 1));

        // ll nx = (x != N - 1 ? x + 1 : 0);
        // ll ny = (y == N - 1 ? 0 : N - 1);
        // mov(nx, ny);

        debug(tako.size(), tar.size(), have, ans.size(), x, y);

        // debug(tako, nex, tar, t);
