...
int main() {
    ...
    vll ans(Q, -1);
    ll now = 0;
    while (now < Q && X[now].first <= N) {
        ans[X[now].second] = A[X[now].first - 1];
        now++;
    }
    ...
    rep(c, N + 1) {
        ...
        while (now < Q && X[now].first <= at + fw.sum(0, M + 1)) {
            ll pl = X[now].first - at;
