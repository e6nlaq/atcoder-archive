...
int main() {
    ...
    for (ll x = L; x <= R; ++x) {
        ...
        if (has_kyotsu(h, {D, U})) {
            h = get_kyotu(h, {D, U});
            sa += (h.second - h.first + 1) * (abs(x) % 2 == 0);

            if (h.first > D) {
                sa += solve(h.first - D, p(x, h.first - 1));
            }

            if (h.second < U) {
                sa += solve(U - h.second, p(x, h.second + 1));
            }
        } else {
            sa += solve(U - D + 1, p(x, D));
        }
        ans += sa;
        debug(x, sa);
    }
