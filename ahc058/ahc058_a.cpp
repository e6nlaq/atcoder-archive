...
CPP_DUMP_DEFINE_EXPORT_OBJECT(State, score, has, trem, apple);

auto operator<=>(const State& l, const State& r) {
    // ll lh = l.score * (T / (T - l.trem));
    // ll rh = r.score * (T / (T - r.trem));
    // return lh <=> rh;
    return l.score <=> r.score;
}
const long long MOD = 5000000007;
const int MAX_C = 560;
...
pair<vvll, ll> step(const vvll& b, const vvll& p, ll t) {
    ll gain = 0;
    vvll next_b = b;
    ...
    // For each machine j
    rep(j, N) {
        // From level src to level dst
        // src > dst
        // B[dst] += B[src] * (Product(P[dst+1..src])) * Com(t, src-dst)

        rep(src, L) {
            if (b[src][j] == 0) continue;

            // To lower levels
            ll prod_p = 1;
            for (int dst = src - 1; dst >= 0; --dst) {
                prod_p *= p[dst + 1][j];
                // Contribution to B[dst]
                // Distance k = src - dst
                // Added = b[src][j] * prod_p * Com[t][k]
                ll ways = Com[t][src - dst];
                if (ways > 0) {
                    next_b[dst][j] += b[src][j] * prod_p * ways;
                }
            }

            // Contribution to Apples (Target Level -1)
            // Distance k = src - (-1) = src + 1
            // P multiplier includes P[0]
            // Current 'prod_p' covers P[1]...P[src]
            ll prod_p_apple = prod_p * p[0][j];
            ll ways_apple = Com[t][src + 1];
            if (ways_apple > 0) {
                gain += b[src][j] * A[j] * prod_p_apple * ways_apple;
            }
        }
    }
...
int main() {
    ...
    auto B = make_vec2<ll>(L, N, 1);
    auto P = make_vec2<ll>(L, N, 0);
    auto R = make_vec2<ll>(L, N, 0);
    R[0] = A;
    ...
    rep(_, T) {
        ...
        rep(i, L) {
            rep(j, N) {
                if (C[i][j] * (P[i][j] + 1) <= K) {
                    q.emplace(R[i][j] + (100 * i * R[i][j]), i, j);
                }
            }
        }
