...
int main()
{
    ...
    // i:モンスター数, j:逃がす逃がさない k:偶奇
    vvvll dp = make_vec3<ll>(N + 1, 2, 2, -INFLL);
    dp[0][0][0] = dp[0][1][0] = 0;
    irep(i, N) {
        // 偶数にガス
        chmax(dp[i][0][0], max(dp[i - 1][0][0], dp[i - 1][1][0]));

        // 奇数にガス
        chmax(dp[i][0][1], max(dp[i - 1][0][1], dp[i - 1][1][1]));

        // ぐうころ
        chmax(dp[i][1][0], max(dp[i - 1][0][1], dp[i - 1][1][1]) + A[i] * 2);

        chmax(dp[i][1][1],max(dp[i-1][0][0],dp[i-1][1][0])+A[i]);
    }
