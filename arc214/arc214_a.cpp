...
int main() {
    ...
    queue<pair<pll, ll>> q;
    q.emplace(pll{N - 1, N - 1}, 0);
    auto pos = make_vec2<ll>(N, N, -1);
    while (!q.empty()) {
        auto [now, a] = q.front();
        q.pop();
        ...
        if (x != 0) q.emplace(pll{x - 1, y}, a + 1);
        if (y != 0) q.emplace(pll{x, y - 1}, a + 1);
    }
    debug(pos);

    vector<set<ll>> dat(2 * N);
    rep(i, N) {
        rep(j, N) {
            if (S[i][j] == '?') continue;

            dat[pos[i][j]].emplace(ctoi(S[i][j]));
        }
    }

    bool ok = true;
    rep(i, 2 * N) ok &= dat[i].size() <= 1;
    if (!ok) {
        co(-1);
    } else {
        rep(i, N) {
            rep(j, N) {
                cout << *dat[pos[i][j]].begin();
            }
            cout << endl;
        }
    }
