...
int main() {
    ...
    while (codeforces_t--) {
        ...
        auto able = make_vec2<bool>(31, 2, false);
        rep(bit, 31) {
            rep(t, 2) {
                vb check(N, false), dat(N, false);
                dat[0] = t;
                check[0] = true;
                queue<ll> q;
                q.emplace(0);
                while (!q.empty()) {
                    ...
                    for (auto [to, c] : G[x]) {
                        ...
                        check[to] = true;
                        dat[to] = dat[x] ^ ((c >> bit) & 1);
                        q.emplace(to);
                    }
                }
                // debug(bit, t, dat);
                ll one = count(all(dat), true);
