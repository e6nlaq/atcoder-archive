...
int main() {
    ...
    while (ans.size() > 1) {
        vpll nex;
        for (ull i = 0; i + 1 < ans.size(); i += 2) {
            nex.emplace_back(make_pair(ans[i].first * ans[i + 1].first + ans[i].second * ans[i + 1].second, ans[i].first * ans[i + 1].second + ans[i].second * ans[i + 1].first));
        }

        if (ans.size() % 2 == 1) {
            nex.emplace_back(ans.back());
        }
