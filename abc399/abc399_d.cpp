...
int main() {
    ...
    while (codeforces_t--) {
        ...
        vb ok(N, false);  // 成立
        rep(i, N) {
            ok[i] = pos[i][0] + 1 == pos[i][1];
        }

        ll ans = 0;
        vector<unset<ll>> dat(N);
        rep(i, N) {
            if (!ok[i]) {
                unset<ll> a, b;
                if (pos[i][0] != 0) a.emplace(A[pos[i][0] - 1]);
                if (pos[i][0] != 2 * N - 1) a.emplace(A[pos[i][0] + 1]);
                if (pos[i][1] != 0) b.emplace(A[pos[i][1] - 1]);
                if (pos[i][1] != 2 * N - 1) b.emplace(A[pos[i][1] + 1]);

                vll tmp;
                for (auto x : a) {
                    if (b.contains(x)) dat[i].emplace(x);
                }
            }
        }
