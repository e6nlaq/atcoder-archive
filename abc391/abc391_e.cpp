...
ll solve(ll x, ll c) {
    ...
    ll b = 3 * c;
    auto s = &dat[x - 1];
    pos[s->at(b)].emplace_back(b);
    pos[s->at(b + 1)].emplace_back(b + 1);
    pos[s->at(b + 2)].emplace_back(b + 2);
    ...
    if (pos.size() == 1) {
        ...
        if (pos['0'].size() == 2) {
            return min(solve(x - 1, pos['0'][0]), solve(x - 1, pos['0'][1]));
        } else {
            return min(solve(x - 1, pos['1'][0]), solve(x - 1, pos['1'][1]));
        }
    }
}
