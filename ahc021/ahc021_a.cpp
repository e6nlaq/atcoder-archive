...
int main()
{
	...
	rep(i, n)
	{
		...
		{
			swap(b[x][y], b[x + 1][y]);
			output[i] = (to_string(x) + " " + to_string(y) + " " + to_string(x + 1) + " " + to_string(y));
			cnt++;
		}
		else if (b[x][y] > b[x + 1][y + 1])
		{
			cnt++;
			swap(b[x][y], b[x + 1][y + 1]);
			output[i] = (to_string(x) + " " + to_string(y) + " " + to_string(x + 1) + " " + to_string(y + 1));
		}
		else if (y != x && b[x][y] < b[x - 1][y])
		{
			cnt++;
			swap(b[x][y], b[x - 1][y]);
			output[i] = (to_string(x) + " " + to_string(y) + " " + to_string(x - 1) + " " + to_string(y));
