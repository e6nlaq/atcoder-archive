...
int main() {
    ...
    // auto dp = make_vec3<ll>(N + 1, N, M + 1, -INFLL);
    // // irep(i, N) {
    // //     rep(j, M + 1) {
    // //         rep(k, K+1) {
    // //            if()
    ...
    //                 if (k + B[j] <= M)
    //                     chmax(dp[i][j][k], dp[i - 1][j - l][k - B[j]] + A[j]);
    //             }
    //         }
    //     }
    // }
    ...
    // ll ans = 0;
    // rep(i, N) {
    //     rep(j, M + 1) chmax(ans, dp[N][i][j]);
    // }
    // co(ans);
    auto dp = make_vec2<ll>(N, M + 1, -INFLL);
    rep(i, N) dp[i][B[i]] = A[i];
