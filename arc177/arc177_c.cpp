...
vvll dic(ll sx, ll sy, char now)
{
	auto kakutei = make_vec2<bool>(N, N, false);
	auto cur = make_vec2<ll>(N, N, INFLL);
	cur[sx][sy] = 0;
	...
	while (!q.empty())
	{
		...
		rep(i, 4)
		{
			ll X = dx[i] + x, Y = dy[i] + y;
			if (!(X >= 0 && X < N && Y >= 0 && Y < N))
				continue;

			ll cost = (c[X][Y] != now);
			if (cur[X][Y] > cur[x][y] + cost)
			{
				cur[X][Y] = cur[x][y] + cost;
				q.push(make_tuple(cur[X][Y], X, Y));
			}
		}
	}
