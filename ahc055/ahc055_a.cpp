...
int main() {
    ...
    vll c = C_orig;
    vb opened(N, false);
    sll available_weapons;
    vector<pll> total_actions;
    ll opened_count = 0;
    ...
    while(opened_count < N) {
        ...
        // Find the best weapon attack
        for (ll w : available_weapons) {
            if (c[w] > 0) {
                rep(b, N) {
                    if (!opened[b]) {
                        if (chmax(max_dmg, A[w][b])) {
                            best_w = w;
                            best_b = b;
                        }
                    }
        ...
        if (max_dmg <= 1) {
            // If no weapon is significantly better than bare hands,
            // we must use bare hands. Choose the target chest wisely.
            w_act = -1;
            ...
            rep(b, N) {
                if (!opened[b]) {
                    if (h[b] <= 0) { // Already open, just needs to be marked
                        target_b = b;
                        break;
                    }
                    ll cost = h[b]; // cost in bare hand attacks
                    
                    ll value_damage = 0;
                    rep(j, N) {
                        if (j != b && !opened[j]) { // Dynamic value calculation
                           value_damage += A[b][j];
                        }
                    }
                    ll value = C_orig[b] * value_damage;
                    
                    ld score = (ld)value / cost;
            ...
            b_act = target_b;
            if (b_act == -1) { // All chests are open
                break;
            }

        } else {
            // A weapon attack is the best option
            w_act = best_w;
            b_act = best_b;
        }

        // Execute the chosen action
        total_actions.push_back({w_act, b_act});
        
        ll dmg_done = (w_act == -1) ? 1 : A[w_act][b_act];
        h[b_act] -= dmg_done;
        ...
        if (h[b_act] <= 0 && !opened[b_act]) {
            opened[b_act] = true;
            available_weapons.insert(b_act);
            opened_count++;
        }
    }

    // Output
    for(const auto& action : total_actions) {
