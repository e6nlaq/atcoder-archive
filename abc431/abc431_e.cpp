...
int main() {
    ...
    while (codeforces_t--) {
        ...
        auto check = make_vec3<ll>(H, W, 4, INFLL);
        vll out(4, INFLL);

        string h = "URDL";
        debug(h);
        queue<tuple<ll, ll, ll>> q;
        auto dfs = [&](ll i, ll j, ll r, ll x) {
            while (i >= 0 && i < H && j >= 0 && j < W && check[i][j][r] == INFLL) {
                check[i][j][r] = x;
                q.emplace(i, j, r);
                debug(i, j);
        ...
        while (!q.empty()) {
...

            {
                ...
                mov(x, y, R);
                if (x >= 0 && x < H && y >= 0 && y < W && check[x][y][R] == INFLL) {
                    dfs(x, y, R, now + 1);
                }
            }
            {
