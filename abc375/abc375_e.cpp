...
pll to(ll moto, ll it, ll po, ll j, ll k) {
    assert(moto != it);
    if (moto == 1) {
        if (it == 2) {
            return {j - po, k + po};
...
int main() {
    ...
    // dp[i][j][k]:=i人目まで移植してに1チームj2チームkパワーにするための最少人数
    auto dp = make_vec3<int>(N + 1, bsum + 1, bsum + 1, INFINT);
    ...
    debug(dp[N]);
    ll las = bsum / 3;
    outif(dp[N][las][las] == INFINT, -1, dp[N][las][las]);
