...
int main() {
    ...
    rep(i, Q) {
        ...
        query[i].first = q;
        query[i].second = vll(q);
        cin >> query[i].second;

        if (q == 1) {
            huusa.emplace(query[i].second[0] - 1);
        }
    }
    ...
    rep(i, M) {
        if (!huusa.contains(i)) {
            g[A[i]].emplace_back(make_cost(B[i], C[i]));
            g[B[i]].emplace_back(make_cost(A[i], C[i]));
        }
    }

    auto dis = warshall_floyd(g);
    reverse(all(query));

    vll ans;
    rep(i, Q) {
        if (query[i].first == 1) {
            ll x = query[i].second[0] - 1;

            chmin(dis[A[x]][B[x]], C[x]);
            chmin(dis[B[x]][A[x]], C[x]);
            debug(dis);

            rep(j, N) {
                rep(k, N) {
                    if (j != k) {
                        chmin(dis[j][k], dis[j][A[x]] + C[x] + dis[B[x]][k]);

                        chmin(dis[j][k], dis[j][B[x]] + C[x] + dis[A[x]][k]);
                    }
                }
            }
            debug(dis);
        } else {
            vll a = query[i].second;
            a[0]--;
            a[1]--;

            ans.emplace_back(dis[a[0]][a[1]] != INFLL ? dis[a[0]][a[1]] : -1);
        }
    }
    reverse(all(ans));
