...
inline map<char, ll> counter(string s) noexcept(except)
{
	map<char, ll> dat;
	rep(i, s.size())
	{
...
ld euclidean(ld x1, ld y1, ld x2, ld y2)
{
	ld dx = x2 - x1;
	ld dy = y2 - y1;
...
pll finda(set<pll> s, ll x, ll y)
{
	ll diff = INFLL;
	pll ans = {-1, -1};
	for (auto [i, j] : s)
	{
		if (chmin(diff, abs(i - x) + abs(j - y)))
			ans = {i, j};
	}
...
ll cnt = 0;
vvll h;
set<pll> minusa, plusa;
void moves(ll tox, ll toy)
{
	debug(tox, toy);
	char x = (tox - posx > 0 ? 'D' : 'U');
	char y = (toy - posy > 0 ? 'R' : 'L');

	if (tox - posx != 0)
	{
		ll sx = (tox - posx) / abs(tox - posx);
		while (tox != posx)
		{
			co(x);
			posx += sx;
...
int main()
{
	...
	while (!minusa.empty())
	{
		auto [mx, my] = finda(minusa, x, y);
		moves(mx, my);

		debug(cnt, -h[mx][my], mx, my);
		while (cnt < -h[mx][my])
		{
			// debug(h, plus, minus);
			auto [px, py] = finda(plusa, x, y);
			moves(px, py);
