...
void output(const vvll &out) {
    co(out.size());
    rep(i, out.size()) {
        cout << out[i][0] << spa << out[i][1] << spa << (out[i][2] == 0 ? 'U' : 'L') << spa << out[i][3] << endl;
    }
    cout << flush;
}

void output(const vvll &out, ll p, ll r, ll d, ll b) {
    co(out.size());
    rep(i, out.size()) {
        if (i != p)
            cout << out[i][0] << spa << out[i][1] << spa << (out[i][2] == 0 ? 'U' : 'L') << spa << out[i][3] << endl;
        else
            cout << p << spa << r << spa << (d == 0 ? 'U' : 'L') << spa << b << endl;
    }
    cout << flush;
}
...
ld randd() {
    ld x = randint(0, 1'000'000'000'000'000'000LL);
    x /= 1e18;
...
ll get_val(const vector<tuple<ll, ll, ll>> &dat, ll x, ll y) {
    ll ans = 0;
    rrep(i, dat.size()) {
        auto [l, r, v] = dat[i];
        if ((l <= x && x <= r) || (l <= y && y <= r) || (x <= l && r <= y)) chmax(ans, v);
    }

    return ans;
}

ll sim(const vvll &ans) {
    vll lh = h, lw = w;
    vector<tuple<ll, ll, ll>> wdat;  // wにたいするh
    vector<tuple<ll, ll, ll>> hdat;
    vpll pos(N);  // (h,w)
    wdat.push_back({0, W - 1, -1});
    hdat.push_back({0, H - 1, -1});
    ...
    rep(i, ans.size()) {
        ...
        if (d == 0) {
            if (b == -1) {
                ...
                sw = pos[b].second + lw[b];
            }
            sh = get_val(wdat, sw, sw + tw - 1);
            pos[p] = {sh, sw};
            wdat.push_back({sw, sw + tw - 1, sh + th - 1});
            hdat.push_back({sh, sh + th - 1, sw + tw - 1});
        } else {
            if (b == -1) {
                sh = 0;
            } else {
    ...
    ll ansh = 0, answ = 0;
    rep(i, N) {
        chmax(ansh, pos[i].first + lh[i]);
        chmax(answ, pos[i].second + lw[i]);
    }
...
int main() {
    ...
    vec2<State> candidate(1);

    // Uシリーズ
    for (ld x = 0.7; x <= 1.5; x += 0.01) {
        ld mok = MOK * x;
        {
            // 大きくw
            ll cnt = 0;
            ...
            vll bk;
            vll down = {0};
            ans.push_back({0, w[0] < h[0], 0, -1});
            cnt += max(w[0], h[0]);
            for (ll i = 1; i < N; i++) {
                down.emplace_back(i);
                if (cnt >= mok) {
                    ans.push_back({i, w[i] < h[i], 0, -1});
                    cnt = max(w[i], h[i]);
                    bk.emplace_back(i - 1);
                    down.clear();
                } else {
                    ans.push_back({i, w[i] < h[i], 0, i - 1});
                    cnt += max(w[i], h[i]);
                }
            }
            ll score = sim(ans);
            candidate[0].push_back({.score = score, .out = ans});
        }
        {
            // 小さくw
            ll cnt = 0;
        ...
        }
        {
            ...
            for (ll i = 1; i < N; i++) {
                down.emplace_back(i);
                if (cnt >= mok) {
                    ans.push_back({i, 0, 0, -1});
                    cnt = w[i];
                    bk.emplace_back(i - 1);
                    down.clear();
                } else {
                    ans.push_back({i, 0, 0, i - 1});
                    cnt += w[i];
                }
            }
            ll score = sim(ans);
    ...
    ll beam = max(25LL, outn);
    while (chrono::duration_cast<std::chrono::milliseconds>(chrono::system_clock::now() - start).count() <= 2700) {
        const auto now = candidate[candidate.size() - 1];
        unset<vvll> dat;

        rep(i, now.size()) dat.emplace(now[i].out);
        ...
        rep(i, now.size()) {
            ...
            tmp[p][1] ^= 1;
            // ll moto = tmp[p][3];
            // tmp[p][3] = randint(-1, p - 1);

            if (!dat.contains(tmp)) {
                candidate.back().push_back({.score = sim(tmp), .out = tmp});
                dat.emplace(tmp);
            }
        ...
        sort(all(candidate.back()), less<State>());
        while (candidate.back().size() > ull(beam)) candidate.back().pop_back();
    }

    cout << "#" << candidate.size() << endl;

    debug(candidate.size(), candidate.back().size());

    rep(i, outn)
        output(candidate.back()[i].out);
