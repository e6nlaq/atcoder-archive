...
int main() {
    ...
    if (K == 1) {
        ...
        ll ans = 0;
        for (ll i = 0; i + 1 < K; i += 2) ans += A[i + 1] - A[i];

        co(ans);
    } else {
        vll x(K), y(K);

        for (ll i = 0; i + 1 < K; i += 2) x[i] = A[i + 1] - A[i];
        for (ll i = 0; i + 1 < K; i += 2) y[i] = ra[i] - ra[i + 1];

        auto X = cum(x), Y = cum(y);
        ...
        ll ans = INFLL;
        for (ll i = 0; i < K; i += 2) chmin(ans, X[i] + Y[i]);
