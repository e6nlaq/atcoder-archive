...
vs S;
    ...
    void dfs(ll x, ll y, ll id) {
        ...
        rep(i,4){
            ...
            if (X >= 0 && X < H && Y >= 0 && Y < W && check[X][Y] == -1&&S[X][Y]=='#') {
                check[X][Y] = id;
                dfs(X,Y,id);
			}
		}
    ...
    rep(i, H) {
        rep(j, W) {
            ...
            rep(k, 4) {
                ...
                if(I>=0&&I<H&&J>=0&&J<W&&check[I][J]!=-1)dat.emplace(check[I][J]);
            }
            debug(i,j,dat,(renk_n)-(dat.size()-1));

            ans+=(renk_n)-(dat.size()-1);
		}
    }
