...
ll bfs(ll x)
{
	...
	while (!q.empty())
	{
		...
		chmax(ans, a.second);
		debug(check, q);
		rep(i, G[a.first].size())
		{
			if (!check[G[a.first][i]])
			{
				check[G[a.first][i]] = true;
				q.push({G[a.first][i], a.second + 1});
			}
		}
		q.pop();
	}
...
int main()
{
	...
	G.resize(N1 + N2, vector<ll>());
	check.resize(N1 + N2, false);
