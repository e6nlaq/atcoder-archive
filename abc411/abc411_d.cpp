...
int main() {
    ...
    map<pair<ll, string>, ll> s2i;
    vector<pair<ll, string>> i2s;
    ...
    while (Q--) {
        ...
        if (query == 1) {
            ...
            pair<ll, string> ndat = {dat[p], s};
            if (!s2i.contains(ndat)) {
                s2i[ndat] = i2s.size();
                i2s.emplace_back(ndat);
            }
    ...
    vector<string> ans;
    while (server != -1) {
        ans.emplace_back(i2s[server].second);
        server = i2s[server].first;
    }
