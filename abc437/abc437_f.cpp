...
CPP_DUMP_DEFINE_EXPORT_OBJECT(S, xmn, xmx, ymn, ymx);

using F = S;

const S def = {INFLL, -INFLL, INFLL, -INFLL};
const F deff = {1, -1, 1, -1};

S op(S a, S b) {
    return {min(a.xmn, b.xmn), max(a.xmx, b.xmx), min(a.ymn, b.ymn), max(a.ymx, b.ymx)};
}
...
int main() {
    ...
    rep(_query, Q) {
        ...
        if (que == 1) {
            ...
            ll X = x - y, Y = x + y;
            seg.set(i, {X, X, Y, Y});
            debug("upd", i, seg.get(i));
        } else {
            ll L, R, X, Y;
            cin >> L >> R >> X >> Y;
            ll x = X - Y, y = X + Y;

            auto dat = seg.prod(L - 1, R);
            debug(dat);

            ll ans = -INFLL;
            chmax(ans, abs(x - dat.xmn));
            chmax(ans, abs(x - dat.xmx));
            chmax(ans, abs(y - dat.ymn));
            chmax(ans, abs(y - dat.ymx));
            co(ans);
        }
#ifdef LOCAL
        rep(i, N)
