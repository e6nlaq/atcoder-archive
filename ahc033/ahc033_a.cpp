...
void update()
{
	...
	rep(i, N)
	{
		rep(j, N - 1)
		{
			if (place[i][j] == -1)
			{
				ana.emplace_back(make_pair(i, j));
			}
		}
	}
}
...
void moves(ll c, ll x, ll y)
{
	tate(c, x - pos[c].first);
	yoko(c, y - pos[c].second);
}

void swp(ll c, ll x1, ll y1, ll x2, ll y2)
{
	assert(place[x2][y2] == -1);
	moves(c, x1, y1);
	ans[c].emplace_back('P');
	moves(c, x2, y2);
	ans[c].emplace_back('Q');
	swap(place[x1][y1], place[x2][y2]);
}
...
int main()
{
	...
	for (ll i = 1; i < N; ++i)
	{
		ans[i].emplace_back('B');
	}

	vector<bool> comp(N * N, false);
	bool changed = false;
	rep(_, 200)
	{
		rep(i, N)
		{
			...
			{
				update();
				if (vals[i * N + j] != make_pair(-1LL, -1LL))
				{
					auto tmp = vals[i * N + j];
					swp(0, tmp.first, tmp.second, i, N - 1);
					comp[i * N + j] = true;
					changed = true;
				}
				else if (!comp[i * N + j])
		...
		{
			update();
			if (!A[i].empty() && !ana.empty())
			{
				auto tmp = ana.back();
				swp(0, i, 0, tmp.first, tmp.second);
				ana.pop_back();
			}
		}
		changed = false;
