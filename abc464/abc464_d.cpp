...
int main() {
    ...
    while (codeforces_t--) {
        ...
        cin >> X >> Y;
        // 0:sunny, 1:rainy
        auto dp = make_vec2<ll>(N + 1, 2, -INFLL);
        dp[0][0] = 0;
        irep(i, N) {
            ll c = S[i - 1] - '0';
            chmax(dp[i][c], max(dp[i - 1][0], dp[i - 1][1]));
            chmax(dp[i][c ^ 1], max(dp[i - 1][0], dp[i - 1][1]) - X[i - 1]);

            if (i > 1)
                chmax(dp[i][0], dp[i - 1][1] + Y[i - 2] - c * X[i - 1]);
        }
        debug(dp);
        co(max(dp[N][0], dp[N][1]));
    }
