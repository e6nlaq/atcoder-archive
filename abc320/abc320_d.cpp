...
void dfs(ll now)
{
	...
	for (auto X : G[now])
	{
		...
		ll b = get<0>(X), x = get<1>(X), y = get<2>(X);
		if (check[b])
			continue;

		pos[b] = make_pair(pos[now].first + x, pos[now].second + y);
		check[b] = true;
		dfs(b);
	}
}
...
int main()
{
	...
	rep(i, M)
	{
		...
		G[A].emplace_back(make_tuple(B, X, Y));
		G[B].emplace_back(make_tuple(A, -X, -Y));
	}
	...
	rep(i, N)
	{
		if (check[i])
		{
			cout << pos[i].first << spa << pos[i].second << endl;
		}
		else
		{
			co("undecidable");
