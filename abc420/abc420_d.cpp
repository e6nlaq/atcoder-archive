...
int main() {
    ...
    auto dis = make_vec3<ll>(2, H, W, INFLL);
    auto check = make_vec3<bool>(2, H, W, false);
    ...
    while (!q.empty()) {
        ...
        rep(i, 4) {
            ...
            if (A[nx][ny] == '#') continue;
            if (A[nx][ny] == "xo"[d]) continue;

            if (A[nx][ny] == '?') {
                nd ^= 1;
            }
            if (check[nd][nx][ny]) continue;
            dis[nd][nx][ny] = dis[d][x][y] + 1;
            check[nd][nx][ny] = true;
            q.emplace(nd, nx, ny);
        }
    }
    ...
    ll ans = min(dis[0][gx][gy], dis[1][gx][gy]);
    if (ans == INFLL) {
        cout << "-1\n";
    } else {
        cout << ans << "\n";
