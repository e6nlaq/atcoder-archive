...
inline void coif(bool a, T1 x, T2 y, string end = "\n")
{
	if (a)
		cout << x << end;
	else
...
int main()
{
	...
	for (int i = 1; i <= N; i++)
	{
		...
		for (auto x : dp[i - 1])
		{
			...
			if (!dp[i].count(tmp))
			{
				dp[i][tmp] = dp[i - 1][x.first] + C;
			}
			else
			{
				dp[i][tmp] = min(dp[i - 1][x.first] + C, dp[i][tmp]);
			}
		}
	}
	...
	if (dp[N].count(vector<ll>(K, P)))
	{
		co(dp[N][vector<ll>(K, P)]);
	}
	else
		co(-1);
