...
int main() {
    ...
    auto G = make_vec2<pair<ll, char>>(N, 0);
    auto R = G;
    rep(i, N) {
        rep(j, N) {
            if (c[i][j] != '-') {
    ...
    auto ans = make_vec2<ll>(N, N, INFLL);
    rep(i, N) {
        rep(j, N) {
            reverse_queue<tuple<ll, ll, ll>> q;  // (cost,s,g)
            ...
            while (!q.empty()) {
                ...
                if (c[x][y] != '-') {
                    // ans[i][j] = 2 * dis + 1;
                    // break;
                    chmin(ans[i][j], 2 * dis + 1);
                    continue;
                }

                for (auto [xt, xc] : G[x]) {
                    for (auto [yt, yc] : R[y]) {
                        if (!check[xt][yt] && xc == yc) {
                            check[xt][yt] = true;
                            q.push({dis + 1, xt, yt});
                            if (xt == yt) {
                                // ans[i][j] = 2 * dis;
    ...
    rep(i, N) rep(j, N) if (ans[i][j] == INFLL) ans[i][j] = -1;
