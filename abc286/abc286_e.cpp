...
struct Val {
   public:
    ...
    friend constexpr bool operator<(const Val &l, const Val &r) {
        if (l.dist == r.dist) return l.cost > r.cost;
        return l.dist < r.dist;
    }

    friend constexpr Val operator+(const Val &l, const Val &r) {
        return {l.dist + r.dist, l.cost + r.cost};
    }
};
CPP_DUMP_DEFINE_EXPORT_OBJECT(Val, dist, cost);
...
int main() {
    ...
    auto d = make_vec2<Val>(N, N, {INFLL, 0});
    rep(i, N) {
        cin >> S;
        rep(j, N) {
            if (S[j] == 'Y') {
