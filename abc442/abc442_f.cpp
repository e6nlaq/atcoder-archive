...
int main() {
    ...
    // debug(S);
    //
    // dp i,j:=i行目の時点で、左からj個以上の白マスを残すためにする最小の手数
    auto dp = make_vec2<ll>(N, N + 1, INFLL);
    rrep(j, N + 1) {
        dp[0][j] = S[0][j] + ((N - j) - (S[0][N] - S[0][j]));
        if (j != N) chmin(dp[0][j], dp[0][j + 1]);
    }
    debug(dp[0]);
    arep(i, 1, N) {
        rrep(j, N + 1) {
            dp[i][j] = S[i][j] + ((N - j) - (S[i][N] - S[i][j])) + dp[i - 1][j];

            if (j != N) chmin(dp[i][j], dp[i][j + 1]);
        }
    }
    debug(dp);
    co(dp[N - 1][0]);
