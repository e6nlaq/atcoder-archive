...
int main()
{
	...
	vector<vector<vector<ll>>> dp(N, make_vec2(2, 2, INFLL));
	// dp[i][x][y] := i文字目がxのつなげたかy
	if (S[0] == '0')
	{
		dp[0][0][0] = 0;
		dp[0][1][0] = C[0];
	...
	for (ll i = 1; i < N; ++i)
	{
		ll x = ctoi(S[i]);
		dp[i][x][0] = dp[i - 1][!x][0];
		dp[i][x][1] = min(dp[i - 1][x][0], dp[i - 1][!x][1]);

		dp[i][!x][0] = dp[i - 1][x][0] + C[i];
		dp[i][!x][1] = min(dp[i - 1][!x][0] + C[i], dp[i - 1][x][1] + C[i]);
	}

	debug(dp);
	co(min(dp[N - 1][0][1], dp[N - 1][1][1]));
