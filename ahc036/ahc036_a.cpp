...
void go(ll to) {
    ...
    dijkstra(G, prv, now);
    vll add={to};
    while (prv[add.back()] != -1) add.emplace_back(prv[add.back()]);

    rrep(i, add.size()) sinro.emplace_back(add[i]);
...
int main()
{
    ...
    t_dist[0] = dijkstra(G,0);

    
    rep(i, T) {
        auto dis = dijkstra(G, t[i]);
        t_dist[t[i]]=dis;
        rep(j, N) {
            point_k[j]+=dis[j];
		}
        ...
        auto pointit = min_element(all(point_k));
        ll p = pointit - point_k.begin();

	t_dist[p]=dijkstra(G,p);
    ...
    rep(i, T) {
        ...
        if(i!=T-1&&t_dist[t[i]][p]<t_dist[t[i]][t[i+1]])
        go(p);
    }

    vll A;
    unset<ll> b;
    for (ull i = 1; i < sinro.size(); ++i) {
        if (!b.contains(sinro[i])) {
            if (b.size() == Lb) {
                b.clear();
			}
            
            A.emplace_back(sinro[i]);
            b.emplace(sinro[i]);
		}
    }

    debug(sinro,A,A.size());

    while (A.size() < La) A.emplace_back(sinro[1]);
    ...
    for (ull i = 1; i < sinro.size(); ++i) {
        if (!tmp.contains(sinro[i])) {
            adds.emplace_back(sinro[i]);
            tmp.emplace(sinro[i]);
		}
    }
    ...
    vb use(jufu.size(),false);
    rep(i, adds.size()) {
        ll sa = INFLL, idx = jufu.size()-i-1;
        // auto dis=dijkstra(G,adds[i]);
        // rep(j, jufu.size()) {
        //     if (!use[j]) {
        //         ll x = jufu[j];
        //         ll l = (x == 0 ? 0: dis[A[x-1]]);
        //         ll r = (x == (ll)La - 1 ? 0 : dis[A[x+1]]);
        //         if (chmin(sa,l+r )) {
        //             idx=j;
		// 		}
		// 	}
    ...
    for (ull i = 1; i < sinro.size(); ++i) {
        if (!b.contains(sinro[i])) {
            b.clear();
            ll aidx = idx[sinro[i]], cnt = 0;

            for (ll j = 0; j + Lb - 1 < La; ++j) {
                unset<ll> dat;
                rep(k, Lb) dat.emplace(A[j+k]);

				ll tmp=0;
                for (ull k = 0; k < Lb && i + k < sinro.size(); ++k) {
                    if (dat.contains(sinro[i + k])) tmp++;
                    else break;
                }

                if (chmax(cnt,tmp))aidx=j;
			}
            ...
            for (ull j = aidx; j < aidx + Lb; ++j) {
                b.emplace(A[j]);
			}
			cout<<"s "<<Lb<<spa<<aidx<<" 0"<<endl;
        }
