...
int main() {
    ...
    ll ans = 0;
    set<pll> big;
    rep(i, K) big.emplace(0, i);
    arep(i, K, N) q.emplace(0, i);
    ...
    while (Q--) {
        ...
        bool eraced = false;
        if (big.contains(make_pair(A[X], X))) {
            big.erase(make_pair(A[X], X));
            ans -= A[X];
            eraced = true;
        }
        A[X] = Y;
        ...
        while (!q.empty() && (big.contains(make_pair(q.top().first, q.top().second)) || q.top().first != A[q.top().second])) {
            q.pop();
        }

        if (!eraced && !big.empty() && big.begin()->first < q.top().first) {
            ans -= big.begin()->first;
            q.emplace(big.begin()->first, big.begin()->second);
            big.erase(big.begin());
        ...
        if (eraced) {
            assert(!q.empty());
            ans += q.top().first;
            big.emplace(q.top().first, q.top().second);
            q.pop();
        }
        debug(big, A);
