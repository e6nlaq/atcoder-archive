...
ll score(ll sx, ll sy, ll ex, ll ey) {
    if (sx > ex) {
        swap(sx, ex);
    }
    ...
    ll scnt = 0;
    auto sab_start = lower_bound(all(sab), make_pair(sx, sy)) - sab.begin();
    for (ll i = sab_start; i < N; ++i) {
        if (sab[i].first <= ex && sy <= sab[i].second && sab[i].second <= ey) {
            scnt++;
        }

        if (sab[i].first > ex) break;
    }

    ll icnt = 0;
    auto iwa_start = lower_bound(all(iwa), make_pair(sx, sy)) - iwa.begin();
    for (ll i = iwa_start; i < N; ++i) {
        if (iwa[i].first <= ex && sy <= iwa[i].second && iwa[i].second <= ey) icnt++;

        if (iwa[i].first > ex) break;
    }

    return scnt - icnt + 1;
}

constexpr ll max_z = 100000;
...
int main() {
    ...
    iwa.resize(N);
    rep(i, N) {
        cin >> sab[i].first >> sab[i].second;
    }

    rep(i, N) {
        cin >> iwa[i].first >> iwa[i].second;
    }
    ...
    ll asx, asy, aex, aey;
    rep(_, 50000) {
        ll sx = randint(0, max_z - 2),
           sy = randint(0, max_z - 2);
        ll ex = randint(sx + 1, max_z - 1), ey = randint(sy + 1, max_z - 1);

        if (chmax(now_score, score(sx, sy, ex, ey))) {
            asx = sx;
            asy = sy;
            aex = ex;
            aey = ey;
    ...
    // 2:右辺
    // 3:下辺
    unmap<pll, pair<pll, ll>> kei;
    kei[{asx, asy}] = {{asx, aey}, 0};
    kei[{asx, aey}] = {{aex, aey}, 1};
    kei[{aex, aey}] = {{aex, asy}, 2};
    kei[{aex, asy}] = {{asx, asy}, 3};
    vpll point = {
        {asx, asy},
        {asx, aey},
        {aex, aey},
    ...
    rep(i, 4) {
        ...
        for (ll _ = 0; _ < temp && add < 1; _++) {
            ...
            if (r == 0) {
                ...
                sx = p1.first;
                sy = randint(p1.second + 1, p2.second - 1);
                ex = randint(0, sx - 1);
                ey = randint(sy + 1, p2.second - 1);
            }
            elif (r == 1) {
                assert(p1.second == p2.second);
                sx = randint(p1.first + 1, p2.first - 1);
                sy = p1.second;
                ex = randint(sx + 1, p2.first - 1);
                ey = randint(sy + 1, max_z);
            }
            elif (r == 2) {
                assert(p1.first == p2.first);
                sx = p1.first;
                sy = randint(p2.second + 1, p1.second - 1);
                ex = randint(sx + 1, max_z);
                ey = randint(p2.second + 1, sy - 1);
            }
            else {
                assert(p1.second == p2.second);
                sx = randint(p2.first + 1, p1.first - 1);
                sy = p1.second;
                ex = randint(p2.first + 1, sx - 1);
                ey = randint(0, sy - 1);
            }
            set<pll> check = {
                {sx, sy},
            ...
            if (check.size() != 4) continue;

            ll add_score = score(sx, sy, ex, ey);

            if (add_score > (temp - _) / a) {
                // スコア加算
                add++;
                now_score += add_score;
                ...
                point.push_back({sx, ey});
                point.push_back({ex, sy});
    ...
    pll now_pos = {asx, asy};
    do {
        ans.emplace_back(now_pos);
        now_pos = kei[now_pos].first;
        debug(now_pos, kei[now_pos]);
    } while (now_pos != make_pair(asx, asy));
