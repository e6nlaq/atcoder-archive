...
inline const CostGraph convert_costgraph(const Graph &g) noexcept
{
...
inline const pair<GraphKey, ll> tree_diamiter_dfs(const CostGraph &G, ll u, ll par)
{ // 最遠点間距離と最遠点を求める
...
inline const vector<ll> dijkstra(const CostGraph &G, ll start, ll init = 0)
{
	ll n = G.size();
...
inline const vector<ll> dijkstra(const Graph &G, ll start, ll init = 0)
{
	return dijkstra(convert_costgraph(G), start, init);
}

inline const vector<vector<ll>> warshall_floyd(const CostGraph &G)
{
	ll n = G.size();
...
inline const vector<vector<ll>> warshall_floyd(const Graph &G)
{
...
int main()
{
	...
	rep(i, M)
	{
		...
		U--;
		V--;
		G[U].emplace_back(make_cost(V, A[V] + B));
		G[V].emplace_back(make_cost(U, A[U] + B));
	}

	auto ans = dijkstra(G, 0, A[0]);
	for (ll i = 1; i < N; ++i)
		cout << ans[i] << spa;
