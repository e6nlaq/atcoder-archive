...
// 直角三角形かどうかを判定する関数
bool isRightTriangle(double ax, double ay, double bx, double by, double cx, double cy)
{
	// 辺の長さを計算
	...
	// ピタゴラスの定理を確認
	return (fabs(AB2 + BC2 - CA2) < 1e-9) || (fabs(AB2 + CA2 - BC2) < 1e-9) || (fabs(BC2 + CA2 - AB2) < 1e-9);
}
...
int main()
{
	...
	YesNo(isRightTriangle(ax, ay, bx, by, cx, cy));
