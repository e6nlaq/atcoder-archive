...
vector<pair<char, ll>> asshuku(string s)
{
	pair<char, ll> tmp = {s[0], 1};
...
int main()
{
	...
	rep(i, tmp.size() - 2)
	{
		if (tmp[i].first == 'J' && tmp[i + 1].first == 'O' && tmp[i + 2].first == 'I' && tmp[i + 1].second <= min(tmp[i].second, tmp[i + 2].second))
		{
			chmax(ans, min(tmp[i].second, min(tmp[i + 1].second, tmp[i + 2].second)));
		}
	}
