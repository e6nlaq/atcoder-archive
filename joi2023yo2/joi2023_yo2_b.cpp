...
ll solve(vll A, vll B, vll C, vll D)
{
	ll ans = INFLL;
	rep(i, N)
	{
		auto b = lower_bound(all(B), A[i]);
		auto c = lower_bound(all(C), A[i]);
		auto d = lower_bound(all(D), A[i]);
		...
		if (d != D.end())
			_d = *d;
		debug(ans, _b, _c, _d, A[i]);

		if (_b == -1 || _c == -1 || _d == -1)
			continue;

		chmin(ans, max(_b, max(_c, _d)) - A[i]);
	}
...
int main()
{
	...
	chmin(ans, solve(B, A, C, D));
	chmin(ans, solve(C, B, A, D));
	chmin(ans, solve(D, A, C, B));
