...
int main() {
    ...
    dat.emplace_back(make_tuple(0, 0, 0));
    ...
    rep(i, N) {
        ...
        auto [p, root, h] = dat.back();
        root += h + 1;

        if (H < root) continue;
        dat.emplace_back(X + root, root, H - root);
    }
    debug(dat);
    ...
    rep(_query, Q) {
        ...
        auto it = upper_bound(all(dat), make_tuple(T, INFLL, INFLL));
        it--;
        debug(*it, T);
        auto [p, root, h] = *it;
        co(min(root + h, root + (T - p)));
    }
