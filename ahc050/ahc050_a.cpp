...
int main() {
    ...
    rep(_, N * N - M) {
        auto nex = make_vec2<ld>(N, N, 0);
        auto cnt = make_vec2<ld>(N, N, 0);
        rep(i, N) {
            rep(j, N) {
                if (S[i][j] == '#' || dat[i][j] == 0) continue;
        ...
        if (ans.first == -1 || _ <= 5 || dat[ans.first][ans.second] < 0.001 || true) {
            ld now = 1e9;
            rep(i, N) {
                rep(j, N) {
                    if (S[i][j] == '.' && cnt[i][j] < now) {
                        ans = {i, j};
                        now = cnt[i][j];
                    }
                }
        ...
        tate[ans.second].insert(ans.first);
        yoko[ans.first].insert(ans.second);
