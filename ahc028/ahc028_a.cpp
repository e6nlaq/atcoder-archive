...
unmap<char, vector<pair<ll, ll>>> dat;
unmap<char, unset<string>> goods;
unset<char> tops;

pair<ll, pair<ll, ll>> chikai_toko(ll x, ll y, char c)
{
	pair<ll, pair<ll, ll>> ans = {INFLL, {-1, -1}};
	rep(i, dat[c].size())
	{
		if (chmin(ans.first, abs(x - dat[c][i].first) + (y - dat[c][i].second) + 1))
		{
			ans.second = {dat[c][i].first, dat[c][i].second};
		}
	}
...
void dfs(ll x, ll y)
{
	...
	if (ans.back() != make_pair(x, y))
	{
		ans.emplace_back(make_pair(x, y));
	}

	pair<string, ll> tmp = make_pair("", INFLL);

	if (goods[A[x][y]].empty())
	{
		return;
	}

	for (auto s : goods[A[x][y]])
	{
		pair<ll, ll> pre = {x, y};
		ll cnt = 0;
		for (ll j = 1; j < 5; ++j)
		{
			auto [cnts, place] = chikai_toko(pre.first, pre.second, s[j]);
			cnt += cnts;
			pre = place;
		}

		if (chmin(tmp.second, cnt))
		{
			tmp.first = s;
		}
	}
	cerr << endl;
	debug(goods['B'], tmp, x, y, A[x][y]);

	auto st = tmp.first;
	goods[A[x][y]].erase(st);
	pair<ll, ll> pre = make_pair(x, y);
	for (ll i = 1; i < st.size(); ++i)
	{
		auto [cnts, place] = chikai_toko(pre.first, pre.second, st[i]);
	...
	if (goods[st.back()].size() != 0)
	{
		dfs(pre.first, pre.second);
	}
	else
	{
		bool flag = false;
		...
		rep(i, 4)
		{
			...
			if (a >= 0 && a < N && b >= 0 && b < N && goods[A[a][b]].size() != 0)
			{
				dfs(a, b);
				flag = true;
		...
		if (!flag)
		{
			pair<ll, pair<ll, ll>> dats = {INFLL, {-1, -1}};
			bool ok = false;
			for (auto c : tops)
			{
				if (goods[c].empty())
				...
				auto [cnts, place] = chikai_toko(_A, _B, c);
				if (chmin(dats.first, cnts))
				{
					dats.second = place;
					ok = true;
				}
			...
			ans.emplace_back(dats.second);
			dfs(dats.second.first, dats.second.second);
		}
	}
}
...
int main()
{
	...
	rep(i, N)
	{
		...
		{
			cin >> A[i][j];
			dat[A[i][j]].emplace_back(make_pair(i, j));
		}
	}
