...
int main() {
    ...
    rep(_query, Q) {
        ...
        if (uf.same(u, v)) {
            debug(_query, id, u, v, id[u], id[v]);
            ok &= id[u] != id[v];
        } else {
            if (uf.size(u) < uf.size(v)) swap(u, v);
            // uのほうが　サイズ　でかい！！！！！！！！！！！！！！！！！！！！！！！！！１
            ll a = uf.leader(u), b = uf.leader(v);
            ...
            ans -= ge(uf.size(u), cnt[a]);
            ans -= ge(uf.size(v), cnt[b]);

            ll ne = uf.merge(u, v);
            cnt[ne] = nex;
            ans += ge(uf.size(ne), cnt[ne]);
        }
        if (!ok) {
            co(-1);
            continue;
