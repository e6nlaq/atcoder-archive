/*------------------------------------------------------------
            ...
            　　∧＿∧        Competitive Programming
            　 ( 　･ω･)      AtCoder, Codeforces and yukicoder
            ＿(__つ/￣￣￣ /
            　　＼/　　　　 /  C++ GCC 14.2.0
            　　　　￣￣￣￣￣
    ...
    |  | V |  I'm not a robot      |
    |  |___|                       |
    |______________________________| XD
...
// https://nyaannyaan.github.io/library/math/isqrt.hpp
...
inline bool is_squere(const ll N) noexcept {
    long long r = isqrt(N);  // 切り捨てした平方根
    return (r * r) == N;
}
...
template <typename T>
inline T emit(const T& a, const T& b, const T& x) {
    assert(a == x ^ b == x);
    if (a == x) return b;
    return a;
}
...
inline bool is_adj(const Point& a, const Point& b) {
    return max(abs(a.r - b.r), abs(a.c - b.c)) <= 1;
}

ll calc_score(const vector<Point>& route) {
    ll sum = 0;
    for (int k = 0; k < N * N; k++) {
        sum += (ll)k * A[route[k].r][route[k].c];
    }
    return sum;
}
...
int main() {
    ...
    vector<pair<int, int>> base_curve;
    for (int d = 0; d < 64; d++) {
        int x, y;
        d2xy(8, d, x, y);
        base_curve.push_back({y, x});
    }

    vector<vector<pair<int, int>>> variations(8);
    for (int r = 0; r < 2; r++) {
        for (int rot = 0; rot < 4; rot++) {
            int v = r * 4 + rot;
            for (auto p : base_curve) {
                ...
                variations[v].push_back({row, col});
            }
        }
    }
    ...
    int bt = 0, bb = N / 8 - 1, bl = 0, br = N / 8 - 1;
    while (bt <= bb && bl <= br) {
        for (int c = bl; c <= br; ++c) blocks.push_back({bt, c});
        bt++;
        for (int r = bt; r <= bb; ++r) blocks.push_back({r, br});
        br--;
        if (bt <= bb) {
            for (int c = br; c >= bl; --c) blocks.push_back({bb, c});
            bb--;
        }
        if (bl <= br) {
            for (int r = bb; r >= bt; --r) blocks.push_back({r, bl});
            bl++;
        }
    }

    int n_blocks = blocks.size();
    vector<vector<int>> dp_v(n_blocks, vector<int>(8, -1));
    for (int v = 0; v < 8; v++) dp_v[0][v] = v;

    for (int i = 0; i < n_blocks - 1; i++) {
        for (int v1 = 0; v1 < 8; v1++) {
            if (dp_v[i][v1] == -1) continue;
            int r1 = blocks[i].first * 8 + variations[v1].back().first;
            int c1 = blocks[i].second * 8 + variations[v1].back().second;
            for (int v2 = 0; v2 < 8; v2++) {
                int r2 = blocks[i + 1].first * 8 + variations[v2].front().first;
                int c2 = blocks[i + 1].second * 8 + variations[v2].front().second;
    ...
    int cur_v = -1;
    for (int v = 0; v < 8; v++) if (dp_v[n_blocks - 1][v] != -1) cur_v = v;

    vector<int> selected_variations(n_blocks);
    for (int i = n_blocks - 1; i >= 0; i--) {
        selected_variations[i] = cur_v;
        cur_v = dp_v[i][cur_v];
    }
    ...
    for (int i = 0; i < n_blocks; i++) {
        int br = blocks[i].first, bc = blocks[i].second;
        for (auto p : variations[selected_variations[i]]) {
            route.push_back({br * 8 + p.first, bc * 8 + p.second});
        }
    }

    for (int i = 0; i < N * N; i++) {
        pos_map[route[i].r][route[i].c] = i;
    }

    ll current_score = calc_score(route);

    mt19937 rng(42);
    uniform_int_distribution<int> dist_i(0, N * N - 1);
    uniform_int_distribution<int> dist_dir(0, 7);

    int iter = 0;
    const int REGION_SIZE = 4;
    uniform_int_distribution<int> dist_r(0, N - REGION_SIZE);
    ...
    while (true) {
        if ((iter & 1023) == 0) {
            ...
            double elapsed = duration_cast<milliseconds>(now - start_time).count();
            if (elapsed > 2850) break;
            temp = T0 + (T1 - T0) * (elapsed / 2850.0);
        }
        iter++;
        ...
        target_indices.reserve(REGION_SIZE * REGION_SIZE);
        for (int dr = 0; dr < REGION_SIZE; dr++) {
            for (int dc = 0; dc < REGION_SIZE; dc++) {
                target_indices.push_back(pos_map[r + dr][c + dc]);
            }
        }
        sort(target_indices.begin(), target_indices.end());

        // try 2-opt within the pieces of path that intersect this REGION_SIZE x REGION_SIZE box
        int n_target = target_indices.size();
        for (int a = 0; a < n_target; a++) {
            for (int b = a + 1; b < n_target; b++) {
                int i = target_indices[a];
                int j = target_indices[b];

                if (i >= j) continue;

                bool ok = true;
                if (i > 0 && !is_adj(route[i - 1], route[j])) ok = false;
                if (j < N * N - 1 && !is_adj(route[i], route[j + 1])) ok = false;
                ...
                for (int k = 0; k <= (j - i) / 2; ++k) {
                    ...
                    int p2 = j - k;
                    if (p1 == p2) continue;
                    diff += (ll)p2 * A[route[p1].r][route[p1].c] + (ll)p1 * A[route[p2].r][route[p2].c] - ((ll)p1 * A[route[p1].r][route[p1].c] + (ll)p2 * A[route[p2].r][route[p2].c]);
                }

                if (diff > 0 || exp((double)diff / temp) > dist_prob(rng)) {
                    for (int k = 0; k <= (j - i) / 2; ++k) {
                        int p1 = i + k;
                        int p2 = j - k;
                        swap(route[p1], route[p2]);
                        pos_map[route[p1].r][route[p1].c] = p1;
                        pos_map[route[p2].r][route[p2].c] = p2;
                    }
                    current_score += diff;
                }
            }
