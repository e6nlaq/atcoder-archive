...
ll solve(const string &s, char c) {
    vll tmp(s.size());
    vll dat = {0};
    rep(i, s.size()) {
        if (s[i] != c) dat.emplace_back(i + 1);
        tmp[i] = s[i] == c;
    }
    dat.emplace_back(s.size() + 1);
    auto Z = cum(tmp);
    ll ans = INFLL;

    rep(i, dat.size() - 1) {
        ll l = dat[i], r = dat[i + 1] - 1;  // <l, r-1<=

        ll L = l + Z[l];
        ll R = Z[s.size()] - Z[r] + (s.size() - r);
