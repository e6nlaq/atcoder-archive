...
int main() {
    ...
    ll m = 0;
    while (d.contains(m)) m++;

    set<ll> ok;
    rep(i, 10) {
        if (!d.contains(i)) ok.insert(i);
    }

    bool full = true;
    rep(i, N.size()) {
        if (!ok.contains(N[i] - '0')) full = false;
    }
    ...
    while (x >= 0 && ok.upper_bound(N[x] - '0') == ok.end()) {
        x--;
    }
    ...
    N[x] = *ok.upper_bound(N[x] - '0') + '0';
    arep(i, x + 1, N.size()) N[i] = m + '0';
