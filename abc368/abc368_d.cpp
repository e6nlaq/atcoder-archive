...
void dfs(ll x) {
    ...
    if(level[root]>level[x])root=x;
    
			if(level[root]<level[x]||(level[root]==level[x]&&root!=x)){
    rep(i, G[x].size()) {
        if (level[G[x][i]] < level[x]&&!check.contains(G[x][i])) {
            dfs(G[x][i]);
		}
	}
}}
...
int main()
{
    ...
    while (!q.empty()) {
        ...
        rep(i, G[x].size()) {
            if (!checklv[G[x][i]]) {
                checklv[G[x][i]] = true;
                q.emplace(make_pair(G[x][i],lv+1));
			}
		}
    }
