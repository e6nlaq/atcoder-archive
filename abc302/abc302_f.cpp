...
int main() {
    ...
    auto sets = make_vec2<ll>(N, 0);
    auto pos = make_vec2<ll>(M, 0);
    rep(i, N) {
        ll A;
        cin >> A;
    ...
    vll dists(N, INFLL);
    distp[0] = 0;
    deque<pair<bool, ll>> q;
    q.emplace_back(false, 0);
    ...
    while (!q.empty()) {
        ...
        if (iss) {
            rep(i, sets[x].size()) {
                if (distp[sets[x][i]] != INFLL) continue;
                distp[sets[x][i]] = dists[x];
                q.emplace_front(false, sets[x][i]);
            }
        } else {
            rep(i, pos[x].size()) {
                if (dists[pos[x][i]] != INFLL) continue;
                dists[pos[x][i]] = distp[x] + 1;
                q.emplace_back(true, pos[x][i]);
            }
        }
    }
