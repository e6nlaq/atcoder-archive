...
int main()
{
	...
	rep(i, M)
	{
		...
		ll pos = B[i];
		ll cnt = A[pos] + plus + seg.get(pos).value;
		A[pos] = -plus;
		seg.set(pos, {0, 1});
		...
		{
			seg.apply(pos + 1, N, 1);
			seg.apply(0, (pos + cnt) - N + 1, 1);
		}
		else
		{
			debug(pos + cnt + 1, pos, cnt);
			seg.apply(pos + 1, pos + cnt + 1, 1);
		}
	}
	...
	rep(i, N)
	{
		debug(seg.get(i).value);
		cout << A[i] + plus + seg.get(i).value << spa;
	}
	cout << endl;
