...
template <typename T>
inline vector<pair<T, ull>> rlencode(const vector<T> &v) noexcept(except) {
    ull n = (ull)v.size();
...
void mov(const vpll &have) {
    ...
    rep(i, have.size() - 1) {
        ll x = have[i + 1].first - have[i].first;
        ll y = have[i + 1].second - have[i].second;
        rep(_, abs(x)) {
            co((x < 0 ? "U" : "D"));
        }
    ...
    ll x = have.back().first;
    ll y = have.back().second;
    rep(_, x) co("U");
    rep(_, y) co("L");
}
...
// @param n 底
// @return log_n(x)
long double log_n(const long double x, const long double n) {
    return logl(x) / logl(n);
}
...
int main() {
    ...
    auto w = make_vec2<ll>(N, N);
    auto d = make_vec2<ll>(N, N);
    cin >> w >> d;

    auto taked = make_vec2<bool>(N, N, false);
    taked[0][0] = true;
    vector<pair<vld, pll>> q;

    rep(i, N) {
        rep(j, N) {
            if (i == 0 && j == 0) continue;
            q.push_back({vld{ld(d[i][j])}, {i, j}});
        }
    }

    sort(all(q));

    debug(q | cpp_dump::back(10));
    ...
    while (!q.empty()) {
        ...
        if (taked[fpos.first][fpos.second]) continue;

        taked[fpos.first][fpos.second] = true;

        vpll have;
        have.push_back(fpos);

        vector<pair<ll, pll>> dat;
        for (ll i = -2; i <= 2; ++i) {
            for (ll j = -2; j <= 2; ++j) {
                if (i == 0 && j == 0) continue;
                if (fpos.first + i < 0 || fpos.first + i >= N || fpos.second + j < 0 || fpos.second + j >= N) continue;
                dat.push_back({w[fpos.first + i][fpos.second + j], pll{fpos.first + i, fpos.second + j}});
            }
        }

        sort(all(dat));

        vll W = {w[fpos.first][fpos.second]};
        vll D = {d[fpos.first][fpos.second]};
        rrep(i, dat.size()) {
            auto [_, pos] = dat[i];
            if (taked[pos.first][pos.second]) continue;

            vll new_w = W;
            bool ok = true;
            vll new_d = D;
            rep(j, D.size()) {
                new_d[j] -= (new_w.back() - new_w[j]) * dist(pos, have.back());
                if (new_d[j] <= 0) ok = false;
            }

            if (!ok) continue;

            new_w.push_back(W.back() + w[pos.first][pos.second]);
            new_d.push_back(d[pos.first][pos.second]);

            rep(j, new_d.size()) {
                ok &= new_d[j] - (new_w.back() - new_w[j]) * dist(pos, pll{0, 0}) > 0;
            }
            ...
            if (ok) {
                ...
                D = new_d;
                have.push_back(pos);
                taked[pos.first][pos.second] = true;

                dat.clear();
                for (ll i = -3; i <= 3; ++i) {
                    for (ll j = -3; j <= 3; ++j) {
                        if (i == 0 && j == 0) continue;
                        if (pos.first + i < 0 || pos.first + i >= N || pos.second + j < 0 || pos.second + j >= N) continue;
                        dat.push_back({i + j + log2(w[pos.first + i][pos.second + j]), pll{pos.first + i, pos.second + j}});
                    }
                }
                sort(all(dat));
                i = dat.size();
