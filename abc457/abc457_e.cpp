...
int main() {
    ...
    rep(_query, Q) {
        ...
        ll cont = 0;
        if (dat.contains(g(S, T))) cont = dat[g(S, T)];
        if (cont >= 2) {
            co("Yes");
        }
        elif (cont == 1) {
            YesNo(t.query(S, T + 1) < T || (S < T && t.query(S + 1, T + 1) <= T));
        }
        else {
            debug(3, _query);
