...
int main()
{
	...
	ll current = get_current(a);

	vector<vvll> s = vector<vector<vector<ll>>>(M, make_vec2<ll>(3, 3));
	rep(i, M)
	{
		cin >> s[i];
	}
	...
	rep(i, K)
	{
		ans[i] = make_tuple(randint(0, M - 1), randint(0, N - 4), randint(0, N - 4));
	}

	ll L = ans.size();
	for (ld t = 1; t <= n; ++t)
	{
		ll repl = randint(0, L - 1);

		ll use = randint(0, M - 1);
		ll i = randint(0, N - 4), j = randint(0, N - 4);
		// current = get_current(a);
		auto old_repl = ans[repl];

		ans[repl] = make_tuple(use, i, j);

		auto calc_tmp = init_a;
		rep(k, L)
		{
			auto [m, p, q] = ans[k];
			rep(l, 3)
			{
				rep(o, 3)
				{
					calc_tmp[p + l][q + o] = (calc_tmp[p + l][q + o] + s[m][l][o]);
				}
			}
		}

		ll newscore = get_current(calc_tmp);

		ld T = get_t(t, n);

		ld prob = expl(min(0.0L, (current - newscore) / T));
		if (newscore > current)
		{
			current = newscore;
		}
