...
int main() {
    ...
    // dat[tx][ty][lx][rx][ly][ry]
    auto dat = make_vec3<vec3<ll>>(3 * H, 3 * W, H + 1, make_vec3<ll>(H + 1, W + 1, W + 1, INFLL));

    auto ztmp = make_vec2<ll>(H, W, 0);
    ll tx, ty;
    rep(i, H) {
        rep(j, W) {
            ztmp[i][j] = S[i][j] == '#';
    ...
    auto Z = cum(ztmp);
    dat[tx + H][ty + W][0][H - 1][0][W - 1] = 0;
    queue<tuple<ll, ll, ll, ll, ll, ll>> q;
    q.emplace(tx + H, ty + W, 0, H - 1, 0, W - 1);
    while (!q.empty()) {
        const auto &V = q.front();
        // ll x = V[0], y = V[1], lx = V[2], rx = V[3], ly = V[4], ry = V[5];
        auto [x, y, lx, rx, ly, ry] = q.front();
        ll here = dat[x][y][lx][rx][ly][ry];
        q.pop();

        if (lx == H || rx == H || ly == W || ry == W) break;
        if (lx > rx || ly > ry) break;

        rep(k, 4) {
            ll X = x + dx[k];
            ll Y = y + dy[k];
            if (!(0 <= X && X < 3 * H && 0 <= Y && Y < 3 * W)) {
                continue;
            }
            ...
            if (0 <= qx && qx < H && 0 <= qy && qy < W) {
                if (lx <= qx && qx <= rx && ly <= qy && qy <= ry && S[qx][qy] == '#') {
                    continue;
                }
            }

            ll LX = min(max(lx, qx - tx), H);
            ll RX = max(min(rx, qx - tx + H - 1), -1LL);
            ll LY = min(max(ly, qy - ty), W);
            ll RY = max(min(ry, qy - ty + W - 1), -1LL);

            if (RX == -1) RX = H;
            if (RY == -1) RY = W;

            if (dat[X][Y][LX][RX][LY][RY] == INFLL) {
                dat[X][Y][LX][RX][LY][RY] = here + 1;

                // if (X == 6 && Y == 6) {
                //     debug(X, Y, LX, RX, LY, RY, qx, qy);
                //     debug(tx, ty, lx, rx, ly, ry);
                // }
                q.emplace(X, Y, LX, RX, LY, RY);
            }
        }
        // debug(q.size());
    }
            ...
            rep(lx, H + 1) {
                rep(rx, H + 1) {
                    rep(ly, W + 1) {
                        rep(ry, W + 1) {
                            bool non = true;
                            if (lx <= rx && ly <= ry && lx != H && rx != H && ly != W && ry != W) {
                                non = cumcnt(Z, lx, ly, rx, ry) == 0;
                            }

                            if (W == 4) {
                                // debug(dat[tx][ty][lx].size());
                            }
                            if (non && dat[tx][ty][lx][rx][ly][ry] != INFLL) {
                                // if (W == 4) {
                                //     debug(tx, ty, lx, rx, ly, ry);
                                // }

                                if (chmin(ans, dat[tx][ty][lx][rx][ly][ry])) {
                                    // debug(tx, ty, lx, rx, ly, ry, ans);
                                }
                            }
                        }
                    }
