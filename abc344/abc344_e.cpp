...
int main()
{
	...
	dat.emplace_back(make_pair(0, 0));
	...
	rep(i, N)
	{
		...
		if (i != N - 1)
			dat.emplace_back(make_pair(A[i], id + 1));
		else
			dat.emplace_back(make_pair(A[i], 0));
	}
	...
	while (Q--)
	{
		...
		if (q == 1)
		{
			...
			id++;
			auto tmp = id_map[x];
			dat.emplace_back(make_pair(y, dat[tmp].second));
			dat[tmp].second = id;
			id_map[y] = id;
		}
		else
	...
	while (pos != 0)
	{
		if (!deleted_id.contains(pos))
		{
			ans.emplace_back(dat[pos].first);
		}
