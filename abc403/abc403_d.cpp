...
int main() {
    ...
    for (auto [x, n] : dat) {
        ...
        while (dat.contains(now) && !check.contains(now)) {
            tmp.push_back(dat[now]);
            check.emplace(now);
            now += D;
        }
        ...
        auto dp = make_vec2<ll>(tmp.size(), 2, INFLL);
        dp[0][0] = tmp[0];
        dp[0][1] = 0;

        arep(i, 1, tmp.size()) {
            dp[i][0] = min(dp[i - 1][0], dp[i - 1][1]) + tmp[i];
            dp[i][1] = dp[i - 1][0];
        }

        ans += min(dp[tmp.size() - 1][0], dp[tmp.size() - 1][1]);
    }

    debug(dat, check, dat.size() == check.size());
    co(ans);
