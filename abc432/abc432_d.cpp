...
bool sam(const sq &a, const sq &b) {
    bool ans = false;

    if (a.lx == b.rx + 1 || a.rx + 1 == b.lx) {
        ans |= has_kyotsu({a.ly, a.ry}, {b.ly, b.ry});
    }

    if (b.ry + 1 == a.ly || a.ry + 1 == b.ly) {
        ans |= has_kyotsu({a.lx, a.rx}, {b.lx, b.rx});
    }

    return ans;
}

CPP_DUMP_DEFINE_EXPORT_OBJECT(sq, lx, rx, ly, ry, size());
...
int main() {
    ...
    rep(_, N) {
        ...
        rep(i, now.size()) {
            ...
            if (C == 'X') {
                pll thi = {x.lx, x.rx};
                if (has_kyotsu(thi, ran) && get_kyotu(thi, ran) == ran) {
                    now.push_back({x.lx, A - 1, x.ly, x.ry});
                    now.push_back({A, x.rx, x.ly, x.ry});
                    continue;
                }

                if (x.rx < A) {
                    nex.push_back({x.lx, x.rx, x.ly - B, x.ry - B});
                } else {
                    nex.push_back({x.lx, x.rx, x.ly + B, x.ry + B});
                }
            } else {
                pll thi = {x.ly, x.ry};
                if (has_kyotsu(thi, ran) && get_kyotu(thi, ran) == ran) {
                    now.push_back({x.lx, x.rx, x.ly, A - 1});
                    now.push_back({x.lx, x.rx, A, x.ry});
                    continue;
                }

                if (x.ry < A) {
                    nex.push_back({x.lx - B, x.rx - B, x.ly, x.ry});
                } else {
                    nex.push_back({x.lx + B, x.rx + B, x.ly, x.ry});
                }
            }
    ...
    rep(i, now.size()) {
        arep(j, i + 1, now.size()) {
            if (sam(now[i], now[j])) G.merge(i, j);
        }
    }
