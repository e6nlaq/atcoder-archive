...
int main() {
    ...
    rep(_query, Q) {
        ...
        if (q == 1) {
            ...
            ll old_le = G.leader(x);
            ll old_c = vcol[old_le];
            ll old_n = G.size(x);

            colorcnt[old_c] -= old_n;

            auto [oldl, oldr] = vlr[old_le];

            debug(x, c, old_c, old_n, colorcnt);

            if (oldl != 0) {
                ll lle = G.leader(oldl - 1);

                if (vcol[lle] == c) {
                    G.merge(oldl, oldl - 1);
                    chmin(oldl, vlr[lle].first);
                }
            }

            if (oldr != N - 1) {
                ll rle = G.leader(oldr + 1);

                if (vcol[rle] == c) {
                    G.merge(oldr, oldr + 1);
                    chmax(oldr, vlr[rle].second);
                }
            }

            ll new_le = G.leader(x);
            vcol[new_le] = c;
            colorcnt[c] += old_n;

            vlr[new_le] = {oldl, oldr};

            debug(x, c, new_le, oldl, oldr, colorcnt);
        } else {
            ll c;
            cin >> c;
