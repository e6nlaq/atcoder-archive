...
int main() {
    ...
    vll ama(N);
    rep(i, dat.size()) {
        ll l = G.leader(dat[i][0]);
        for (auto x : dat[i]) {
            ama[l] += jcnt[x];
        }
    }

    auto hon = make_vec2<ll>(N, 0);
    rep(i, juh.size()) {
        hon[G.leader(A[juh[i]])].emplace_back(juh[i]);
    }
    ...
    while (q.size() > 1) {
        ...
        queue<ll> us(all(hon[x]));
        hon[x].clear();

        while (!q.empty() && !us.empty()) {
            auto y = q.front();
            q.pop();
            ...
            // us.push(all(hon[y]));
            rep(i, hon[y].size()) us.push(hon[y][i]);
            hon[y].clear();

            ans.push_back({t + 1, A[t] + 1, y + 1});
