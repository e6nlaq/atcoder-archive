...
int main() {
    ...
    irep(i, N) cin >> X[i] >> Y[i] >> Z[i];

    ll sumz = sum(Z);
    auto dp = make_vec2<ll>(N + 1, sumz + 1, INFLL);
    dp[0][0] = 0;

    irep(i, N) {
        ll p = max((X[i] + Y[i] + 1) / 2 - X[i], 0LL);
        rep(j, sumz + 1) {
            chmin(dp[i][j], dp[i - 1][j]);
            chmin(dp[i][j + Z[i]], dp[i - 1][j] + p);
        }
    }
    ...
    ll ans = INFLL;
    for (ll i = (sumz + 1) / 2; i <= sumz; ++i) chmin(ans, dp[N][i]);
