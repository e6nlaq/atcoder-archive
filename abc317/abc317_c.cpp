...
ll ans = 0;
vector<vector<pair<ll, ll>>> G(19, vector<pair<ll, ll>>());

/* Main Function */

ll dfs(ll pos, ll load, vector<bool> check)
{
	check[pos] = true;
	...
	for (auto x : G[pos])
	{
		...
		if (!check[x.second])
		{
			chmax(tmp, dfs(x.second, load + x.first, check));
		}
	}
...
int main()
{
	...
	{
		cerr << endl;
		chmax(ans, dfs(i, 0, vector<bool>(N, false)));
	}
