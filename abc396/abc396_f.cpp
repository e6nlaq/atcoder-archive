...
int main() {
    ...
    vll big(N, 0);
    rep(i, N) {
        now += i - fw.sum(0, A[i] + 1);
        tuika[i] = i - fw.sum(A[i], A[i] + 1);
        fw.add(A[i], 1);
    }

    vll ref(N, 0);
    {
        fenwick_tree<ll> dat(M);
        rrep(i, N) {
            dat.add(A[i], 1);
            ref[i] = dat.sum(0, A[i]);
            big[i] = (N - i) - dat.sum(0, A[i] + 1);
        }
    }

    debug(now, ref, tuika, big);
    ...
    rep(i, N) {
        ...
        ll x = M - A[i];
        diff[x] += tuika[i] - ref[i] - big[i];
    }
    debug(diff);

    assert(diff[0] == 0);
    diff[0] = now;
    arep(i, 1, M) diff[i] += diff[i - 1];
    print(diff, "\n");
