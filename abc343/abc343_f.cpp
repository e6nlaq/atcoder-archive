...
{
	map<ll, ll> tmp;
	tmp[a.first.first] += a.first.second;
	tmp[a.second.first] += a.second.second;
	tmp[b.first.first] += b.first.second;
	tmp[b.second.first] += b.second.second;

	debug(tmp, a, b);
	if (tmp.size() == 1)
	{
		pp ans = {{tmp.begin()->first, tmp.begin()->second}, {0, 0}};
		debug(ans);
		return ans;
	}
	else
	{
		pp ans = {{prev(tmp.end())->first, prev(tmp.end())->second}, {prev(tmp.end(), 2)->first, prev(tmp.end(), 2)->second}};
		debug(ans);
		return ans;
	}
}
