...
int main() {
    ...
    do {
        for (ll bit = 0; bit < (1LL << (N - 1)); ++bit) {
            ...
            bitset<8> b(bit);
            rep(i, N) {
                group.back().emplace_back(dat[i]);
                if (i < N - 1 && b.test(i)) {
                    group.emplace_back(vll{});
                }
            }
            ...
            vector<unset<ll>> tonari(N);
            rep(i, group.size()) {
                tonari[group[i][0]].emplace(group[i][1]);
                tonari[group[i][0]].emplace(group[i][group[i].size() - 1]);
                tonari[group[i][group[i].size() - 1]].emplace(group[i][0]);
                tonari[group[i][group[i].size() - 1]].emplace(group[i][group[i].size() - 2]);

                arep(j, 1, group[i].size() - 1) {
                    tonari[group[i][j]].emplace(group[i][j - 1]);
                    tonari[group[i][j]].emplace(group[i][j + 1]);
                }
            }
            ...
            rep(x, N) {
                ll i = dat[x];
                for (auto [to, e] : G[i]) {
                    if (!tonari[i].contains(to) && !erased.contains(e)) {
                        erased.emplace(e);
                    }
                }
            }
            ...
            rep(x, N) {
                ...
                for (auto to : tmp) {
                    pll edge = make_pair(min(i, to), max(i, to));
            ...
            chmin(ans, erased.size() + added.size());
        }
