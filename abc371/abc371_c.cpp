...
int main()
{
    ...
    cin >> N;
    vector<unset<ll>> G(N,unset<ll>()),H(N,unset<ll>());
    ...
    debug(A,tai,N);
        ...
        rep(i, N) {
            rep(j, N) {
                ...
                if (G[i].contains(j) ^ h[tai[i]].contains(tai[j])) {
                    cnt += A[tai[i]][tai[j]];
                    if (h[tai[i]].contains(tai[j])) {
                        h[tai[i]].erase(tai[j]);
                        h[tai[j]].erase(tai[i]);
                    } else {
                        h[tai[i]].emplace(tai[j]);
                        h[tai[j]].emplace(tai[i]);
					}
				}
			}
        }
        ...
        chmin(ans,cnt);
        } while (next_permutation(all(tai)));
