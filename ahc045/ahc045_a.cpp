...
vector<pll> query(const vector<ll> &c) {
    cout << "? " << c.size();
    for (ll val : c) {
        cout << " " << val;
    }
...
void answer(const vector<vector<ll>> &groups, const vector<vector<pll>> &edges) {
    cout << "!" << endl;
    cout.flush();
...
int main() {
    ...
    //     ll tmp = INFLL;
    //     rep(i, N) {
    //         if (chmin(tmp, dist({0LL, 0LL}, pll{x[i], y[i]}))) fir = i;
    //     }
    //     debug(fir, tmp);
    // }
    ...
    //         ll nex = -1;
    //         rep(i, N) {
    //             if (!check[i] && chmin(d, dist({x[i], y[i]}, {x[now], y[now]}))) {
    //                 nex = i;
    //             }
    //         }
    //         cities[t + 1] = nex;
    ...
    // sort(cities.begin(), cities.end(), [&](ll a, ll b) {
    ...
    // ll start_idx = 0;
    // for (ll g : G) {
    //     groups.push_back(vector<ll>(cities.begin() + start_idx, cities.begin() + start_idx + g));
    //     start_idx += g;
    // }
    ll score = INFLL;
    vvll now;
    rep(mota, 10) {
        vvll groups(M);
        reverse_queue<pll> root;
        ll cscore = 0;
        ll mot = randint(0, N - 1);
        rep(i, N) {
            // root.emplace(dist(pll{0, 0}, pll{x[i], y[i]}), i);
            root.emplace(dist(pll{x[mot], y[mot]}, pll{x[i], y[i]}), i);
            // root.emplace(randint(0, INFLL), i);
        }

        vb check(N, false);
        rep(m, M) {
            ll g = G[m];
            while (check[root.top().second]) root.pop();
            ll f = root.top().second;
            reverse_queue<tuple<ll, ll, ll>> q;
            q.emplace(0, f, -1);
            while (!q.empty() && g > 0) {
                auto [_, i, prv] = q.top();
                q.pop();
                ...
                check[i] = true;
                groups[m].emplace_back(i);
                g--;

                if (prv != -1) {
                    cscore += dist({x[i], y[i]}, {x[prv], y[prv]});
                }

                rep(j, N) {
                    if (!check[j]) q.emplace(dist({x[i], y[i]}, {x[j], y[j]}), j, i);
                }
            }
        }

        if (chmin(score, cscore)) now = groups;
    }

    auto groups = now;
    debug(groups);

    vector<vector<pll>> edges(M);
    for (ll k = 0; k < M; ++k) {
        for (ll i = 0; i < G[k]; i += (L - 1)) {
            ll l = min(G[k] - i, L);
            if (l >= 3) {
                // vector<ll> subset = {groups[k][i], groups[k][i + 1], groups[k][i + 2]};
                vll subset(groups[k].begin() + i, groups[k].begin() + i + l);
                vector<pll> ret = query(subset);
