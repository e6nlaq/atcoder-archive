...
int main()
{
	...
	vector<vector<ll>> b(m, vector<ll>(n / m));
	vcin(b);
	...
	for (int del = 1; del <= n; ++del)
	{
		...
		if (up == -1)
		{
			out.emplace_back(make_pair(del, 0));
			b[place].pop_back();
			continue;
		}
		else
		{
			...
			rep(i, m)
			{
				if (place != i && chmin(siz, b[i].size()))
				{
					to = i;
				}
			}

			out.emplace_back(make_pair(b[place][up], to + 1));

			ll cnt = 0;
			for (int i = up; i < b[place].size(); i++)
			{
				b[to].emplace_back(b[place][i]);
				cnt++;
			}
	...
	rep(i, min((ll)out.size(), 5000LL))
	{
		cout << out[i].first << spa << out[i].second << endl;
	}
