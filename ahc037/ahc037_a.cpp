...
int main()
{
    ...
    vll A(N + 1), B(N + 1);
    A[0]=B[0]=0;
    irep(i, N) cin >> A[i] >> B[i];
	N++;

	vvll G=make_vec2<ll>(N,0);
        ...
        rep(j, N) {
            if (i!=j&&A[i] >= A[j] && B[i] >= B[j] && chmin(dis, (A[i] - A[j]) + (B[i] - B[j]))) {
                x=j;
			}
        }
        G[x].emplace_back(i);
    ...
    rep(i, N) {
        if (G[i].size() >= 2) {
			...
			vll repl;
            for (ull j = 0; j < G[i].size(); j += 2) {
				if(j+1==G[i].size())break;

            ll x = min(A[srt[j].second], A[srt[j + 1].second]), y = min(B[srt[j].second], B[srt[j + 1].second]);

            // if(!db.contains(make_pair(x,y))){
            ll nidx = A.size();
            G.emplace_back(vll{srt[j].second,srt[j+1].second});
            A.emplace_back(x);
            B.emplace_back(y);
            repl.emplace_back(nidx);
            db[{x,y}]=nidx;
            pl++;
            // } else {
                // ll nidx = db[{x, y}];
            ...
            if (srt.size() % 2) {
                repl.emplace_back(srt.back().second);
            }
            G[i]=repl;
		}
    }
    ...
    while (!q.empty()) {
        ...
        rep(i, G[x].size()) {
            if (!check[G[x][i]]) {
                check[G[x][i]] = true;
                ans.emplace_back(make_pair(x,G[x][i]));
                q.push(G[x][i]);
			}
		}
    }
    ...
    vvll print_ans;
    rep(i, ans.size()) {
        vll out={A[ans[i].first],B[ans[i].first],A[ans[i].second],B[ans[i].second]};
        if (!exist_ans.contains(out)) {
            print_ans.emplace_back(out);
            exist_ans.emplace(out);
        }
    }
