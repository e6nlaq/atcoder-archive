...
int main() {
    ...
    rep(i, M) {
        ...
        G[A].push_back({B, {false, C}});
        G[B].push_back({A, {true, C}});
    }

    // auto check = make_vec2<bool>(N, M + 1, false);
    auto dist = make_vec2<ll>(N, M + 1, INFLL);
    ...
    while (!q.empty()) {
        ...
        for (auto [to, cost] : G[x]) {
            if (flip == M && cost.first) continue;
            if (chmin(dist[to][flip + cost.first], dist[x][flip] + cost.second)) {
                q.emplace(dist[to][flip + cost.first], to, flip + cost.first);
            }
        }
    }
