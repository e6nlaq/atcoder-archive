...
constexpr int STATION = 5000;
constexpr int RAIL = 100;
...
// https://zenn.dev/reputeless/books/standard-cpp-for-competitive-programming/viewer/dijkstra#1.2-%E6%9C%80%E7%9F%AD%E7%B5%8C%E8%B7%AF%E3%82%92%E5%86%8D%E6%A7%8B%E7%AF%89%E3%81%99%E3%82%8B
void Dijkstra(const Status &st, std::vector<long long> &distances, pll startIndex, pll targetIndex, std::vector<pll> &path) {
    // 直前の頂点を記録する配列
    std::vector<ll> p(distances.size(), -1);

    // 「現時点での最短距離, 頂点」の順に取り出す priority_queue
    // デフォルトの priority_queue は降順に取り出すため std::greater を使う
    // std::priority_queue<Pair, std::vector<Pair>, std::greater<Pair>> q;
    reverse_queue<pair<ll, pll>> q;
    q.emplace((distances[to_1(startIndex)] = 0), startIndex);
    ...
    while (!q.empty()) {
        ...
        // 最短距離でなければ処理しない
        if (distances[to_1(from)] < distance) {
            continue;
        }
        ...
        rep(k, 4) {
            pll to = {from.first + dx[k], from.second + dy[k]};
            if ((!(0 <= to.first && to.first < N && 0 <= to.second && to.second < N)) || (!st.stations.contains(to) && st.placed.contains(to))) {
                continue;
            }

            // to までの新しい距離
            const long long d = (distances[to_1(from)] + 1);

            // d が現在の記録より小さければ更新
            if (d < distances[to_1(to)]) {
                // 直前の頂点を記録
                p[to_1(to)] = to_1(from);

                q.emplace((distances[to_1(to)] = d), to);
            }
        }
    }

    // targetIndex に到達できれば最短経路を再構築する
    if (distances[to_1(targetIndex)] != INFLL) {
        for (int i = to_1(targetIndex); i != -1; i = (p[i])) {
            path.push_back(make_pair(i / N, i % N));
        }
...
void tunag(pll from, pll to, Status &st) {
    vll dis(N * N, INFLL);
    vpll path(0);
    Dijkstra(st, dis, from, to, path);
    // rep(i, path.size() - 1) {
    // }
    // debug(path);
    map<pair<char, char>, ll> rails = {
        {{'R', 'R'}, 1},
        {{'R', 'U'}, 4},
        {{'R', 'D'}, 3},
        {{'L', 'L'}, 1},
        ...
        {{'D', 'R'}, 5}};
    // debug(path.size());
    arep(i, 1, path.size() - 1) {
        // 既に駅
        if (st.stations.contains(path[i])) continue;

        char a = movec(path[i - 1], path[i]);
        char b = movec(path[i], path[i + 1]);

        // 建設
        st.money -= RAIL;
        while (st.money < 0) {
            st.action.push_back({-1, 0, 0});
            st.money += st.step;
            st.turn++;
        }
        st.action.push_back({rails[{a, b}], path[i].first, path[i].second});
        st.placed.emplace(path[i]);
...
void place_station(pll pos, Status &st) {
    // 建設
    st.money -= STATION;
    while (st.money < 0) {
        st.action.push_back({-1, 0, 0});
    ...
    st.action.push_back({0, pos.first, pos.second});
    st.stations.emplace(pos);
    st.placed.emplace(pos);

    ll x = pos.first;
    ll y = pos.second;

    if (0 <= x && x < N && 0 <= y && y < N) {
        for (auto v : works[x][y]) {
            if (!st.work.contains(v)) {
                st.work.emplace(v);
                if (st.home.contains(v)) {
                    st.step += dis[v];
                }
            }
        }

        for (auto v : homes[x][y]) {
            if (!st.home.contains(v)) {
                st.home.emplace(v);
                if (st.work.contains(v)) {
                    st.step += dis[v];
    ...
    st.turn++;
    // debug(st.turn, st.step, st.money);
}

pll tikai_station(pll x, const Status &st) {
    ll dis = INFLL;
    pll ans = {-1, -1};
    vll dists(N * N, INFLL);
    vpll path(0);
    Dijkstra(st, dists, x, x, path);

    // rep(i, st.size()) {
    for (auto station : st.stations) {
        if (chmin(dis, dists[to_1(station)])) {
            ans = station;
        }
    }
    // assert(dis != INFLL);
...
pll tikai_many(pll pos, const Status &st) {
    ll cnt = -1;
    pll ans = pos;
    rep(i, siba.size()) {
        ll X = pos.first + siba[i].first;
        ll Y = pos.second + siba[i].second;
        if (0 <= X && X < N && 0 <= Y && Y < N) {
            ll now = 0;
            for (auto x : homes[X][Y]) {
                if (!st.home.contains(x)) now++;
            }
            for (auto x : works[X][Y]) {
                if (!st.work.contains(x)) now++;
            }

            if (chmax(cnt, now)) ans = {X, Y};
        }
    }
    return ans;
}
...
template <typename T>
void set_mapdata(vec3<T> &dat, ll x, ll y, T val) {
    if (0 <= x && x < N && 0 <= y && y < N) {
        dat[x][y].emplace_back(val);
    }
}
...
int main() {
    ...
    for (ll i = -2; i <= 2; i++) {
        for (ll j = -2; j <= 2; j++) {
            if (abs(i) + abs(j) <= 2) siba.push_back({i, j});
        }
    }
    debug(siba);
    ...
    works = homes = make_vec3<ll>(N, N, 0);
    ...
    rep(i, M) {
        ...
        rep(j, siba.size()) {
            auto [x, y] = siba[j];
            // home[s[i].first+x][s[i].second+y]
            set_mapdata(homes, s[i].first + x, s[i].second + y, i);
            set_mapdata(works, g[i].first + x, g[i].second + y, i);
        }
    }

    const ll beamK = 30;
    auto beam = make_vec2<Status>(1, 1);
    beam[0][0] = {
        K,
        0,
        K,
    ...
    ll t = 1;
    while (chrono::duration_cast<std::chrono::milliseconds>(chrono::system_clock::now() - start).count() <= 2880) {
        beam.push_back({});
        // debug(t);
        rep(i, beam[t - 1].size()) {
            auto dat = beam[t - 1][i];
            ...
            beam[t].emplace_back(dat);

            if (!dat.inited) {
                ll sc = -1;
                ll x = -1;
                rep(i, M) {
                    if (dat.home.contains(i) && dat.work.contains(i)) continue;

                    auto cost = ((dis[i]) * RAIL) + STATION * 2;
                    if (cost <= dat.money) {
                        if (chmax(sc, dis[i])) x = i;
                    }
                }
                debug(sc, x, dat.money, ((dis[x] - 1) * RAIL) + STATION * 2);

                // if (sc > dat.money || dis[i] + dat.turn > T) {
                //     continue;
                // }
                ...
                dat.inited = true;
                // debug(dat.action, dat.money, dat.turn);
                tunag(s[x], g[x], dat);
                debug(dat.money, dat.step);
                place_station(s[x], dat);
                place_station(g[x], dat);
                debug(sc, x);
                // dat.step += dis[x];
                // dat.money += dis[x];
                dat.inited = true;
                // dat.ok.emplace(x);
            } else if (dat.home.size() < uint(M) || dat.work.size() < uint(M)) {
                // ll x = randint(0, M - 1);
                // while (dat.ok.contains(x)) x = randint(0, M - 1);
                ll sc = 0;
                ll x = randint(0, M - 1);
                if (randint(0, 2) == 0) {
                    rep(i, M) {
                        ...
                        // auto cost=(dis[i] - 2) * RAIL) + STATION * 2;
                        if (chmax(sc, dis[i])) x = i;
                    }
                } else {
                    while (dat.home.contains(x) && dat.work.contains(x)) {
                        x = randint(0, M - 1);
                        // debug(x, dat.home.size(), dat.work.size());
                    }
                }

                if (!dat.home.contains(x)) {
                    auto c = s[x];
                    // auto [x, y] = c;
                    // ll k = randint(0, siba.size() - 1);
                    // while (!(0 <= x + siba[k].first && x + siba[k].first < N && 0 <= y + siba[k].second && y + siba[k].second < N)) k = randint(0, siba.size() - 1);
                    // c.first += siba[k].first;
                    // c.second += siba[k].second;
                    c = tikai_many(c, dat);
                    auto stt = tikai_station(c, dat);

                    if (stt == make_pair(-1LL, -1LL)) {
                        continue;
                        debug("donma");
                    }

                    if (!dat.placed.contains(c)) {
                        tunag(c, stt, dat);
                    }
                    place_station(c, dat);
                }

                if (!dat.work.contains(x)) {
                    auto c = g[x];
                    // auto [x, y] = c;
                    // ll k = randint(0, siba.size() - 1);
                    // while (!(0 <= x + siba[k].first && x + siba[k].first < N && 0 <= y + siba[k].second && y + siba[k].second < N)) k = randint(0, siba.size() - 1);
            ...
            dat.final = dat.money + dat.step * (T - dat.turn);
            // debug(dat.money, dat.step, dat.final);
        ...
        sort(all(beam[t]), greater<Status>());
        while (beam[t].size() > beamK) beam[t].pop_back();
    ...
    const auto ans = &beam.back()[0];
    debug(ans->action.size(), ans->stations.size(), ans->inited);
    debug(ans->final, ans->step, beamK);

    ll nowt = beam.size() - 1;
    ll now = 0;
    vector<tll> out;
    while (now != -1) {
        rrep(i, beam[nowt][now].action.size()) {
            out.emplace_back(beam[nowt][now].action[i]);
        }
        now = beam[nowt][now].prv;
        nowt--;
        // debug(now);
    }

    reverse(all(out));
    while (out.size() < ull(T)) {
        out.push_back({-1, 0, 0});
    }

    assert(out.size() == ull(T));
    rep(i, T) {
        auto [f, x, y] = out[i];
        // debug(out[i]);
        if (f == -1) {
