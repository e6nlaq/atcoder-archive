...
int main() {
    ...
    // dp[i][b]:=i+1までの動物と(b&&N番目の動物)にえさを与えた時の費用の最小値
    auto dp = make_vec2<ll>(N, 2, INFLL);
    ...
    rep(i, N) {
        rep(j, 2) {
            if (i > 1)
                chmin(dp[i][j], dp[i - 2][j] + A[i - 1]);
            if (i > 0) {
                chmin(dp[i][j], dp[i - 1][j] + A[i]);
                if (i != N - 1)
                    chmin(dp[i + 1][j], dp[i - 1][j] + A[i]);
            }
        }
    }
    debug(dp);
    // co(min({dp[N - 2][0] + A[N - 2], dp[N - 2][1], dp[N - 1][0], dp[N - 1][1]}));
    co(min({dp[N - 2][1], dp[N - 1][0], dp[N - 1][1]}));
