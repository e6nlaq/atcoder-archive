...
bool has_kuka(pll l, pll r) {
    ...
    // if (x) return has_kuka(r, l, false);

    return false;
}

pll kyotu(pll l, pll r) {
...
bool isok(ll m) {
    ...
    rep(i, N) {
        ll kezu = (U[i] + D[i]) - m;
        ll l = max(0LL, U[i] - kezu);
        ll r;
        {
            ll noko = max(0LL, kezu - D[i]);
            r = U[i] - noko;
        }
        debug(i, l, r, m, kezu);
        ...
        if (!ok) break;
        auto [okl, okr] = kyotu(prv, {l, r});
        prv = make_pair(okl - X, okr + X);
    }
