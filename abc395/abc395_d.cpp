...
int main() {
    ...
    vll hato(N);  // 鳩がいる場所 鳩iが左からhato[i]番目の箱にいる。
    iota(all(hato), 0);

    vll hako_idx(N);  // 今その箱がある場所 箱iが左からhako_idx[i]番目にある
    iota(all(hako_idx), 0);

    vll ba(N);  // 左からi番目は箱ba[i]
    iota(all(ba), 0);
    ...
    rep(_query, Q) {
        ...
        if (op == 1) {
            ...
            swap(ba[hako_idx[a]], ba[hako_idx[b]]);
            swap(hako_idx[a], hako_idx[b]);
        }
        else {
            ll a;
            cin >> a;
