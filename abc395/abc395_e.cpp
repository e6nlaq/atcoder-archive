...
int main() {
    ...
    auto G = make_vec2<pair<ll, bool>>(N, 0);
    rep(i, M) {
        ll u, v;
        cin >> u >> v;
    ...
    reverse_queue<tuple<ll, bool, ll>> q;
    q.push({0, false, 0});

    // vb kaku(N, false);
    auto kaku = make_vec2<bool>(2, N, false);
    // vll dist(N, INFLL);
    auto dist = make_vec2<ll>(2, N, INFLL);

    while (!q.empty()) {
        auto [c, b, x] = q.top();
        q.pop();

        if (kaku[b][x]) continue;

        kaku[b][x] = true;
        dist[b][x] = c;
        for (auto [t, tb] : G[x]) {
            if (chmin(dist[tb][t], c + 1 + X * (b ^ tb))) {
                q.push({dist[tb][t], tb, t});
            }
        }
    }

    co(min(dist[0][N - 1], dist[1][N - 1]));
