...
int main()
{
	...
	while (T--)
	{
		vector<pll> dat;
		vll sum;
		auto x = make_vec2<ll>(2 * N * (N - 1), M);
		cin >> x;

		rep(i, x.size())
		{
			ll tmp = accumulate(all(x[i]), 0);
			dat.push_back({tmp, i});
		...
		vector<bool> used(dat.size(), false);
		auto out = make_vec2<ll>(N, N, -1);

		out[0][0] = dat[0].second;
		used[dat[0].second] = true;
		...
		for (ll _x = 0; _x < N; _x++)
		{
			rep(_y, N)
			{
				...
				for (ll i = 0; i < dat.size(); ++i)
				{
					...
					rep(j, M)
					{
						mx += max(x[pos][j], x[i][j]);
						mn += min(x[pos][j], x[i][j]);
						// if (x[pos][j] <= x[i][j])
						// {
						// 	cnt += x[i][j] - x[pos][j];
						// }
					...
					usedat.emplace_back(make_pair(randint(mn, mx), i));
				}

				sort(rall(usedat));

				ll usecnt = 0;
				if (_y != N - 1 && out[_x][_y + 1] == -1)
				{
					out[_x][_y + 1] = usedat[usecnt].second;
					used[out[_x][_y + 1]] = true;
					usecnt++;
				}
				if (_x != N - 1 && out[_x + 1][_y] == -1)
				{
					out[_x + 1][_y] = usedat[usecnt].second;
					used[out[_x + 1][_y]] = true;
					usecnt++;
				}
				debug(usecnt);
			}
