...
int main() {
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            rep(i, N) {
                rep(j, N) {
                    chmin(d[i][j], d[i][x] + d[x][y] + d[y][j]);
                    chmin(d[i][j], d[i][y] + d[y][x] + d[x][j]);
                }
            }
        }
        elif (t == 2) {
            ...
            rep(_, 2) {
                ...
                rep(k, N) {
                    rep(i, N) {
                        chmin(d[i][x], d[i][k] + d[k][x]);
                    }
                }

                rep(k, N) {
                    rep(j, N) {
                        chmin(d[x][j], d[x][k] + d[k][j]);
                    }
                }
            }
        }
