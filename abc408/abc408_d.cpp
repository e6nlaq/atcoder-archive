...
int main() {
    ...
    while (T--) {
        ...
        auto dp = make_vec2<ll>(N + 1, 3, INFLL);  // 0,1,2
        dp[0][0] = 0;
        irep(i, N) {
            dp[i][0] = dp[i - 1][0] + ctoi(S[i - 1]);
            dp[i][1] = min(dp[i - 1][0], dp[i - 1][1]) + (!ctoi(S[i - 1]));
            dp[i][2] = min({dp[i - 1][0], dp[i - 1][1], dp[i - 1][2]}) + ctoi(S[i - 1]);
        }

        debug(S, dp);

        co(min({dp[N][0], dp[N][1], dp[N][2]}));
    }
