...
int main() {
    ...
    lazy_segtree<S, op, e, F, mapping, composition, id> seg1(vll(N, -INF)), seg2(vll(N, -INF));
    vll pos(N);
    rep(i, N) {
        pos[H[i] - 1] = i;
    }
    ...
    arep(i, D, N) {
        seg1.set(pos[i - D], seg2.get(pos[i - D]));
        ll l = max(pos[i] - R, 0LL);
        ll r = min(pos[i] + R, N - 1);
        ll val = seg1.prod(l, r + 1);
        seg2.set(pos[i], (val == -INF ? 0 : val + 1));
