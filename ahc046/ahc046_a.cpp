...
// (succes?, move)
pair<bool, vs> bfs(pll start, pll to, const vpll &walls) {
    vector<set<ll>> yoko(N);  // yoko[i]:=i行目にある壁
    vector<set<ll>> tate(N);  // tate[i]:=i列目にある壁
    rep(i, N) {
        yoko[i].insert(-1);
        yoko[i].insert(N);
        tate[i].insert(-1);
    ...
    auto dis = make_vec2<ll>(N, N, INFLL);
    auto prv = make_vec2<pair<pll, string>>(N, N, {make_pair(-1, -1), ""});
    auto check = make_vec2<bool>(N, N, false);
    rep(i, walls.size()) {
        assert(start != walls[i]);
        ll x = walls[i].first;
        ll y = walls[i].second;
        yoko[x].insert(y);
        tate[y].insert(x);
    ...
    queue<pair<ll, pll>> q;  // (距離,pos,移動データ)
    q.push(make_pair(dis[start.first][start.second], start));
    while (!q.empty() && !check[to.first][to.second]) {
        auto [dist, pos] = q.front();
        q.pop();

        dis[pos.first][pos.second] = dist;
        ...
        rep(k, 4) {
            ...
            if (0 <= X && X < N && 0 <= Y && Y < N && !check[X][Y]) {
                ...
                check[X][Y] = true;
                prv[X][Y] = make_pair(pos, "M " + string{ido[k]});
            }
        }
        ...
        rep(k, 4) {
            ...
            if (!check[X][Y]) {
                q.push(make_pair(dist + 1, make_pair(X, Y)));
                check[X][Y] = true;
                prv[X][Y] = make_pair(pos, "S " + string{ido[k]});
            }
        }
    }

    if (!check[to.first][to.second]) {
        return {false, {}};
    }
    ...
    pll now = to;
    // debug(prv);
    while (prv[now.first][now.second].first != make_pair(-1LL, -1LL)) {
        ans.emplace_back(prv[now.first][now.second].second);
        now = prv[now.first][now.second].first;
    }
...
pair<bool, vs> solve(const vpll &pos, const vpll &walls, pll now, ll nex) {
    vs ans;
    for (ll i = nex; i < M; ++i) {
        auto [ok, dat] = bfs(now, pos[i], walls);
...
int main() {
    ...
    auto [_, ans] = solve(pos, {}, pos[0], 1);
    assert(_);
    ll score = sol_score(ans.size());

    ll t = 0;
    while (chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - start).count() <= 1980) {
        t++;
        auto nas = ans;
        // rep(_, randint(0, nas.size())) nas.pop_back();
        ll axt = t + randint(max(-15LL, -t), 0);
        ll tr = ll(nas.size()) - axt - 1LL;
        rep(_, (tr <= 0 ? randint(1, min(180LL, ll(nas.size()))) : tr)) nas.pop_back();

        vpll walls;
        auto wcheck = make_vec2<bool>(N, N, false);
        rep(i, M) {
            wcheck[pos[i].first][pos[i].second] = true;
        }
        pll now = pos[0];
        ll nex = 0;
        {
            ...
            rep(i, nas.size()) {
                ...
                if (a == 'M') {
                    now.first += movx[d].first;
                    now.second += movx[d].second;
                }
                elif (a == 'S') {
                    ll &X = now.first;
                    ll &Y = now.second;
        ...
        for (ll k : kxx) {
            ...
            if (0 <= X && X < N && 0 <= Y && Y < N && !wcheck[X][Y]) {
                walls.emplace_back(X, Y);
                wcheck[X][Y] = true;
                nas.emplace_back("A " + string{ido[k]});
                break;
            }
        }

        auto [ok, dat] = solve(pos, walls, now, nex);
        if (!ok) continue;

        rep(i, dat.size()) {
            nas.emplace_back(dat[i]);
        }

        debug(score, sol_score(nas.size()));
        if ((score < sol_score(nas.size()))) {
            ans = nas;
            score = sol_score(nas.size());
        }
    }
    debug(t);
