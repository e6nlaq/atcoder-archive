...
constexpr int mas = 2;
static_assert(20 % mas == 0);

struct Color {
    ld c, m, y;
};

using Wel = pair<Color, ll>;

ld cdist(Color a, Color b) {
    return (a.c - b.c) * (a.c - b.c) + (a.m - b.m) * (a.m - b.m) + (a.y - b.y) * (a.y - b.y);
}
vector<Color> own, target;

Wel cmerge(Wel a, Wel b) {
    Color c = {0, 0, 0};
    c.c += a.first.c * a.second;
    c.m += a.first.m * a.second;
    c.y += a.first.y * a.second;
    c.c += b.first.c * b.second;
    c.m += b.first.m * b.second;
    c.y += b.first.y * b.second;

    c.c /= a.second + b.second;
    c.m /= a.second + b.second;
    c.y /= a.second + b.second;

    return {c, a.second + b.second};
}

bool operator<(const Wel &a, const Wel &b) {
    return cdist(a.first, {0, 0, 0}) < cdist(b.first, {0, 0, 0});
}
...
int main() {
    ...
    {
        vll dat(N - 1);
        rep(i, dat.size()) dat[i] = (i % mas == mas - 1);
    ...
    auto c = make_vec2<Wel>(N / mas, N / mas);

    // 処理
    rep(i, H) {
        set<Wel> check;
        queue<tuple<ll, pll, Wel, vll>> q;
        rep(x, N / mas) {
            rep(y, N / mas) {
                if (!check.contains(c[x][y])) {
                    q.push({0, {x, y}, c[x][y], {}});
                    check.emplace(c[x][y]);
                }
            }
        ...
        ld best_score = 1e18;
        while (!q.empty()) {
            ll max_turn = min({(T - H) / H, 10000LL / D, 5LL});
            auto [turn, pos, s, set] = q.front();
            q.pop();

            auto [x, y] = pos;

            auto this_score = cdist(s.first, target[i]);
            if (this_score > 100 * best_score) continue;

            if (!set.empty() && chmin(best_score, this_score)) {
                best_set = set;
                best_pos = pos;
                best_wel = s;
            }
            ...
            if (s.second < mas * mas) {
                rep(j, K) {
                    Wel new_wel = cmerge(s, {own[j], 1});
                    if (!check.contains(new_wel)) {
                        vll new_set = set;
                        new_set.emplace_back(j);
                        q.push({turn + 1, pos, new_wel, new_set});
                        check.emplace(new_wel);
                    }
                }
            }
            ...
            // 捨て
            if (s.second == mas * mas) {
                Wel new_wel = {s.first, s.second - 1};
                if (!check.contains(new_wel)) {
                    vll new_set = set;
                    new_set.emplace_back(-1);
                    q.push({turn + 1, pos, new_wel, new_set});
                    check.emplace(new_wel);
                }
            }
        ...
        rep(j, best_set.size()) {
            if (best_set[j] == -1) {
                cout << "3 " << best_pos.first * mas << " " << best_pos.second * mas << endl;
            } else {
                cout << "1 " << best_pos.first * mas << " " << best_pos.second * mas << " " << best_set[j] << endl;
            }
        }
        c[best_pos.first][best_pos.second] = best_wel;
        cout << "2 " << best_pos.first * mas << " " << best_pos.second * mas << endl;
        c[best_pos.first][best_pos.second].second--;
    }
