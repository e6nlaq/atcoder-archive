...
struct zat {
   ...
   public:
    /// @brief 要素を追加する (exec前のみ利用可能) O(1)
    void add(const T& x) {
        assert(!executed);
        xs.push_back(x);
    }

    /// @brief 座標圧縮を実行する O(N log N)
    void exec() {
        assert(!executed);
    ...
    /// @brief 元の値xの圧縮後インデックスを返す (exec後のみ利用可能) O(log N)
    ...
    /// @brief 圧縮後インデックスiの元の値を返す (exec後のみ利用可能) O(1)
    T at(size_t i) const {
        assert(executed);
    ...
    /// @brief 圧縮後の要素数を返す (exec後のみ利用可能) O(1)
    size_t size() const {
        assert(executed);
...
S op(const S& a, const S& b) {
    // if (a == e()) return b;
    // if (b == e()) return a;
    // return {a.l, b.r, max({a.ans, a.l, a.r + b.l, b.r, b.ans})};
    //

    if (a.tone && b.tone) {
        int x = a.l + b.r;
        return {x, x, max({x, a.ans, b.ans}), true};
    }
    if (a.tone && !b.tone) {
        return {a.l + b.l, b.r, max({a.l + b.l, b.ans}), false};
    }
    if (!a.tone && b.tone) {
        return {a.l, a.r + b.l, max({b.ans, a.l, a.r + b.l}), false};
    }
    return {a.l, b.r, max({a.ans, a.l, a.r + b.l, b.r, b.ans}), false};
}
...
int main() {
    ...
    scanf("%d %d", &N, &Q);
    scanf("%s", s);
    vector<segtree<S, op, e>> seg(26, segtree<S, op, e>(N));

    debug(N, Q);
    rep(i, N) {
        ll c = s[i] - 'a';
        seg[c].set(i, {1, 1, 1, true});
    }
    ...
    rep(_query, Q) {
        ...
        if (que == 1) {
            ...
            seg[s[i] - 'a'].set(i, {0, 0, 0, false});
            seg[x].set(i, {1, 1, 1, true});
            s[i] = x + 'a';
        } else {
            int l, r;
            scanf("%d %d", &l, &r);
