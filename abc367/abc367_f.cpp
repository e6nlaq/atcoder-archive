...
/// @return
inline ll
randint(const ll l, const ll r) noexcept(except)
{
	if (l == r)
		return l;
	return l + (rand() % (r - l));
...
int main()
{
    ...
    vll hash(N);
    rep(i, N) {
        hash[i]=randint(1, mod);
    }
    

    vll ZA(N + 1), ZB(N + 1);
    irep(i, N) {
        ZA[i] = (ZA[i - 1] + hash[A[i - 1] - 1]) % mod;
        ZB[i]=(ZB[i - 1] + hash[B[i - 1]-1])%mod;
    }
    ...
    rep(_query, Q)
	{
        ...
        YesNo((ZA[r]-ZA[l-1])%mod==(ZB[R]-ZB[L-1])%mod);
	}
