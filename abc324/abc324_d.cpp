...
/// @param n x^nのn
/// @return x^n
inline unsigned long long pow_ll(long long x, long long n) noexcept(except)
{
...
int main()
{
	...
	while (true)
	{
		...
		{
			debug(i, now[i], flag);
			if (tmp.find(now[i]) != tmp.end())
			{
				tmp.erase(tmp.find(now[i]));
			}
			else
			{
				flag = false;
