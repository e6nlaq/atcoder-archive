...
int main()
{
	...
	cin >> N;
	vector<pair<ll, ll>> dat(N, make_pair(0, 0));
	rep(i, N)
	{
	...
	pair<ll, ll> tmp = make_pair(-1, -1);
	rep(i, N)
	{
		if (dat[i].second != dat[0].second)
		{
			tmp = dat[i];
			break;
		}
	...
	if (tmp.first != -1)
	{
		co(max(dat[0].first + tmp.first, dat[0].first + (dat[1].first / 2)));
	}
	else
	{
		co(dat[0].first + (dat[1].first / 2));
	}
