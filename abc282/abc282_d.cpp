...
int main()
{
	...
	{
		vll cnts(2);
		cnts[0] = count(all(check), 0);
		cnts[1] = N - cnts[0];

		auto gr = g.groups();
		vvll gcnts = make_vec2<ll>(gr.size(), 2, 0);
		vll pos(N);
		rep(i, gr.size())
		{
			rep(j, gr[i].size())
			{
				gcnts[i][check[gr[i][j]]]++;
				pos[gr[i][j]] = i;
			}
		}
		...
		rep(i, N)
		{
			ans += gcnts[pos[i]][!check[i]] - G[i].size();
			ans += N - (gcnts[pos[i]][0] + gcnts[pos[i]][1]);
		}
