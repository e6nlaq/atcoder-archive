...
int main()
{
	...
	while (!A.empty())
	{
		if (!check.contains(make_pair(A.front() - 1, B.front() - 1)))
		{
			ans.emplace_back(make_pair(A.front(), B.front()));
			check.emplace(make_pair(A.front() - 1, B.front() - 1));
			A.pop();
			B.pop();
		}
		else
