...
pair<ll, vll> get_score(vll roots) {
    vb check(N, false);
    ll ans = 0;
    ll n = N;
    vll trr;
    ...
    if (flag)
        ...
        return get_score(trr);
}

pair<vector<unset<ll>>, vll> get_ans(const vll &roots) {
    vb check(N, false);
    vll dist(N, 0);
    // vvll ans(N);
    vector<unset<ll>> ans(N);
    ll n = N;
    for (auto i : roots) {
        if (check[i]) continue;
...
int main() {
    ...
    vec2<Beam> beams(1);
    beams[0].push_back({init_roots, get_score(init_roots).first});
    ll k = 10;

    while (chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - start).count() <= 1800) {
        beams.push_back({});
        const auto &b = beams[beams.size() - 2];

        for (const auto &[roots, score] : b) {
            beams.back().push_back({roots, score});
            // 変更
            {
                vll new_roots = roots;
                ll x = randint(0, roots.size() - 1);
                ll v = randint(0, N - 1);
                new_roots[x] = v;
                auto [new_score, _] = get_score(new_roots);
                new_roots = _;
                if (new_score > score) {
                    beams.back().push_back({new_roots, new_score});
                }
            }
            ...
            {
                vll new_roots = roots;
                ll x = randint(0, new_roots.size() - 1);
                ll y = randint(0, new_roots.size() - 1);

                swap(new_roots[x], new_roots[y]);
                auto [new_score, _] = get_score(new_roots);
                new_roots = _;
                if (new_score > score) {
                    beams.back().push_back({new_roots, new_score});
            ...
            {
                vll new_roots = roots;
                new_roots.emplace_back(randint(0, N - 1));
        ...
        sort(all(beams.back()), greater<Beam>());
        while (beams.back().size() > (ull)k) beams.back().pop_back();
    }

    auto roots = beams.back()[0].roots;
    auto [ans, dis] = get_ans(roots);
    ...
    while (!q.empty()) {
        ...
        rep(i, G[x].size()) {
            ll t = G[x][i];
            if (dis[t] + 1 <= H && chmax(now, dis[t] + 1)) {
                new_p = t;
            }
        }
    ...
    sort(all(roots));
    debug(roots);
    debug(k, beams.size(), roots.size(), beams.back()[0].score);
