...
int main()
{
	...
	vector<vector<ull>> D(N + 1, vector<ull>(N + 1));
	for (int i = 1; i <= N; ++i)
	{
		for (int j = 1; j <= N; ++j)
		{
	...
	vector<vector<ull>> dp(2, vector<ull>(N + 1, INFLL));
	dp[0][1] = 0;
	dp[1][1] = 0;
	...
	while (!q.empty())
	{
		...
		for (int j = 1; j <= N; ++j)
		{
			debug(i, j, dp[0][j], dp[0][i] + D[i][j] * A);
			if (chmin(dp[0][j], dp[0][i] + (D[i][j] * A)))
			{
				q.push(j);
			}

			if (chmin(dp[1][j], min(dp[0][i] + (D[i][j] * B + C), dp[1][i] + (D[i][j] * B + C))))
			{
				q.push(j);
			}
		}
