...
inline void print(const vector<pair<A, B>> &v, string s = "\n") noexcept(except)
{
...
int main()
{
	...
	vll plus, plusg, ng, ngg;
	rep(i, N)
	{
		if (S[i] == '1')
		{
			plus.emplace_back(X[i]);
			plusg.emplace_back(X[i] + T);
		}
		else
		{
			ng.emplace_back(X[i]);
			ngg.emplace_back(X[i] - T);
		}
	}
	...
	rep(i, plus.size())
	{
		ans += upper_bound(all(ng), plus[i] + 2ll * T) - lower_bound(all(ng), plus[i]);
	}

	rep(i, ng.size())
	{
		// ans += cnts(plus, ng[i] - T * 2, ng[i]);
		ans += upper_bound(all(plus), ng[i]) - lower_bound(all(plus), ng[i] - 2LL * T);
	}
